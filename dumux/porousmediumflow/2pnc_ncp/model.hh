...
#ifndef DUMUX_2PNC_NCP_MODEL_HH
#define DUMUX_2PNC_NCP_MODEL_HH
...
struct TwoPNCNCPModelTraits
{
    using Indices = TwoPNCNCPIndices;

    static constexpr int numEq() { return nComp+numFluidPhases(); }
...
namespace TTag {
struct TwoPNCNCP { using InheritsFrom = std::tuple<PorousMediumFlow>; };
struct TwoPNCNCPNI { using InheritsFrom = std::tuple<TwoPNCNCP>; };
} // end namespace TTag
...
struct LocalResidual<TypeTag, TTag::TwoPNCNCP> { using type = TwoPNCNCPLocalResidual<TypeTag>; };

//! The primary variables vector for the 2pnc model
template<class TypeTag>
struct PrimaryVariables<TypeTag, TTag::TwoPNCNCP>
{
private:
...
template<class TypeTag>
struct VolumeVariables<TypeTag, TTag::TwoPNCNCP>
{
private:
...
public:
    using type = TwoPNCNCPVolumeVariables<NCTraits<BaseTraits, DT, EDM>>;
};

//! Set the base model traits
template<class TypeTag>
struct BaseModelTraits<TypeTag, TTag::TwoPNCNCP>
{
private:
...
public:
    using type = TwoPNCNCPModelTraits<FluidSystem::numComponents,
...
template<class TypeTag>
struct ModelTraits<TypeTag, TTag::TwoPNCNCP> { using type = GetPropType<TypeTag, Properties::BaseModelTraits>; }; //!< default the actually used traits to the base traits

//! Set the vtk output fields specific to this model
template<class TypeTag>
struct IOFields<TypeTag, TTag::TwoPNCNCP> { using type = TwoPNCNCPIOFields; };


template<class TypeTag>
struct ReplaceCompEqIdx<TypeTag, TTag::TwoPNCNCP> { static constexpr int value = GetPropType<TypeTag, Properties::FluidSystem>::numComponents; }; //!< Per default, no component mass balance is replaced

//! Default formulation is pw-Sn, overwrite if necessary
template<class TypeTag>
struct Formulation<TypeTag, TTag::TwoPNCNCP>
{ static constexpr auto value = TwoPFormulation::p0s1; };

template<class TypeTag>
struct SetMoleFractionsForFirstPhase<TypeTag, TTag::TwoPNCNCP> { static constexpr bool value = true; };  //!< Set the primary variables mole fractions for the wetting or nonwetting phase
template<class TypeTag>
struct UseMoles<TypeTag, TTag::TwoPNCNCP> { static constexpr bool value = true; };                         //!< Use mole fractions in the balance equations by default

//! Use the model after Millington (1961) for the effective diffusivity
template<class TypeTag>
struct EffectiveDiffusivityModel<TypeTag, TTag::TwoPNCNCP> { using type = DiffusivityMillingtonQuirk<GetPropType<TypeTag, Properties::Scalar>>; };

//! This model uses the compositional fluid state
template<class TypeTag>
struct FluidState<TypeTag, TTag::TwoPNCNCP>
{
private:
...
template<class TypeTag>
struct ModelTraits<TypeTag, TTag::TwoPNCNCPNI>
{
private:
...
//! Set the volume variables property
template<class TypeTag>
struct VolumeVariables<TypeTag, TTag::TwoPNCNCPNI>
{
private:
    using PV = GetPropType<TypeTag, Properties::PrimaryVariables>;
    using FSY = GetPropType<TypeTag, Properties::FluidSystem>;
...
public:
    using type = TwoPNCNCPVolumeVariables<NCNITraits<BaseTraits, DT, EDM, ETCM>>;
};

//! Set non-isothermal output fields
template<class TypeTag>
struct IOFields<TypeTag, TTag::TwoPNCNCPNI> { using type = EnergyIOFields<TwoPNCNCPIOFields>; };

//! Somerton is used as default model to compute the effective thermal heat conductivity
template<class TypeTag>
struct ThermalConductivityModel<TypeTag, TTag::TwoPNCNCPNI>
{
private:
    using Scalar = GetPropType<TypeTag, Properties::Scalar>;
public:
