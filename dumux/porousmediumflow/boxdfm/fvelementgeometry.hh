...
public:
   ...
   scvs(const BoxDfmFVElementGeometry& fvGeometry, const LocalDof& localDof)
   {
       return std::views::iota(std::size_t(0), fvGeometry.numScv())
            | std::views::filter([fvGeometry, localDof](size_t i) { return fvGeometry.scv(i).localDofIndex() == localDof.index(); })
            | std::views::transform([fvGeometry](size_t i) {return  fvGeometry.scv(i); });
   }
