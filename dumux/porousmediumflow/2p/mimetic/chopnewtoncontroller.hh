 ...
 * way out of bounds.
 */
#ifndef DUMUX_2P_CHOP_STAGGERED_NEWTON_CONTROLLER_HH
#define DUMUX_2P_CHOP_STAGGERED_NEWTON_CONTROLLER_HH
...
class TwoPChopStaggeredNewtonController : public StaggeredNewtonController<TypeTag>
{
    typedef StaggeredNewtonController<TypeTag> ParentType;
...
public:
    TwoPChopStaggeredNewtonController(const Problem &problem)
        : ParentType(problem)
    {}
...
private:
    ...
    {
        for (unsigned int i = 0; i < uLastIter[cellCenterIdx].size(); ++i) {
            saturationChop_(uCurrentIter[cellCenterIdx][i][saturationIdx],
                            uLastIter[cellCenterIdx][i][saturationIdx]);
