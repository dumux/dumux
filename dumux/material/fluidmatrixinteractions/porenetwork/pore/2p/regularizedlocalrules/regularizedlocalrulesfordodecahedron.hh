 ...
 */
#ifndef REGULARIZED_PNM_2P_LOCAL_RULES_FOR_DODECAHEDRON_HH
#define REGULARIZED_PNM_2P_LOCAL_RULES_FOR_DODECAAHEDRON_HH
...
class RegularizedTwoPLocalRulesDodecahedron : public RegularizedTwoPLocalRulesBase
{
    using LocalRules = TwoPLocalRulesDodecahedron<ScalarT>;
...
public:
    ...
    {   // TODO!!!!
        assert(0 <= sw && sw <= 1);
        assert(params.shape == Pore::Shape::tetrahedron);
        using std::exp;
        const Scalar sigma = params.surfaceTension;
        const Scalar poreRadius = params.poreRadius;
        const Scalar e = exp(3.87*sw);
        return -(7.74*sigma*e) / (poreRadius*(e-1.0)*(e-1.0));
    }
