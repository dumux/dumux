 ...
 */
#ifndef DUMUX_GEOMECHANICS_POROELASTIC_SPATIAL_PARAMS_HH
#define DUMUX_GEOMECHANICS_POROELASTIC_SPATIAL_PARAMS_HH
...
class SpatialParamsPoroElasticImpl;

/*!
 * \ingroup SpatialParameters
 * \brief The base class for spatial parameters of linear elastic poro-mechanical problems.
 ...
 * \note Currently, the poroelastic model works with the box and finite element schemes. The interfaces
...
using SpatialParamsPoroElastic
= SpatialParamsPoroElasticImpl<Scalar, GridGeometry, Implementation, GridGeometry::discMethod>;
 ...
 */
template<class Scalar, class GridGeometry, class Implementation>
class SpatialParamsPoroElasticImpl<Scalar, GridGeometry, Implementation, DiscretizationMethod::box>
 ...
 */
template<class Scalar, class GridGeometry, class Implementation>
class SpatialParamsPoroElasticImpl<Scalar, GridGeometry, Implementation, DiscretizationMethod::fem>
: public SpatialParamsElasticBase<Scalar, GridGeometry, Implementation>
{
    using ParentType = SpatialParamsElasticBase<Scalar, GridGeometry, Implementation>;
...
public:
                    ...
                    const ElementSolution& elemSol) const
    {
        ...
        return this->asImp_().porosityAtPos(ipData.ipGlobal());
    }
    ...
    Scalar inertVolumeFraction(const Element& element,
                               ...
                               const ElementSolution& elemSol,
                               int compIdx) const
    { return 1.0 - this->asImp_().porosity(element, ipData, elemSol); }

    // specialization if there are no inert components at all
    template<class SolidSystem, class IpData, class ElementSolution, typename std::enable_if_t<SolidSystem::numInertComponents == 0, int> = 0>
    Scalar inertVolumeFraction(const Element& element,
                               const IpData& ipData,
                               const ElementSolution& elemSol,
                               int compIdx) const
                                  ...
                                  int compIdx) const
    {
        ...
        return this->asImp_().template reactiveVolumeFractionAtPos<SolidSystem>(ipData.ipGlobal(), compIdx);
    }
                           ...
                           const SecondaryVariables& secVars) const
    {
        ...
        return this->asImp_().biotCoefficientAtPos(ipData.ipGlobal());
    }
};
