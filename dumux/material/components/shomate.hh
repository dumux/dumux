...
{
public:
    ShomateMethod(std::vector<Scalar> tempRange, std::vector<std::vector<Scalar>> coeffs)
    ...
    std::vector<Scalar> paramsAtTemperature(const Scalar& T) const
    {
        ...
        if (T < tempRange_[0])
        {
            if (!warningPrinted_)
            {
                std::cerr << "Temperature "<< T << " [K] is out of range. Enthalpy values might be extrapolated." << std::endl;
                       ...
                       const Scalar& pressure) const
    {
        ...
        [[maybe_unused]]Scalar G = correctCoeffs[6];
...
private:
        ...
        std::vector<std::vector<Scalar>> coeffs_;
