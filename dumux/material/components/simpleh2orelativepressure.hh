...
public:
    ...
    {
        return
            gasEnthalpy(temperature, pressure+1e5) -
    ...
    static const Scalar liquidInternalEnergy(Scalar temperature,
                                             Scalar pressure)
    { return
            liquidEnthalpy(temperature, pressure+1e5) -
            (pressure+1e5)/liquidDensity(temperature, pressure+1e5); }
    ...
    {
        // Assume an ideal gas
        return molarMass()*IdealGas::molarDensity(temperature, pressure+1e5);
    }
     ...
     * \param pressure pressure of component in \f$\mathrm{[Pa]}\f$
     */
    static Scalar liquidDensity(Scalar temperature, Scalar pressure)//pressure dependent density which we added recently because it didnt work for the previous system
    {
        Scalar pl = (pressure+1e5)/1e6;//not sure if here I should also add 1e5
...
template <class Scalar>
const Scalar SimpleH2ORelPress<Scalar>::R = Constants<Scalar>::R / 18e-3;
