 ...
 *          to calculate thermodynamically meaningful quantities.
 */
template <class Scalar>
class BlackOil
    : public Base< Scalar, BlackOil<Scalar> >
    {
        using Spline = Dumux::Spline<Scalar, -1>;
        using SplineSamplingPoints = std::vector<std::pair<Scalar, Scalar> >;
        using ThisType = BlackOil<Scalar>;
    ...
    static std::string phaseName(const int phaseIdx)
    {
        assert(0 <= phaseIdx && phaseIdx < numPhases + 1);
        switch (phaseIdx)
        {
            case wPhaseIdx: return "w";
            case oPhaseIdx: return "o";
    ...
    static std::string componentName(const int compIdx)
    {
        //static const char *name[] = { "O", "W", "G" };
        assert(0 <= compIdx && compIdx < numComponents);
        switch(compIdx)
        {
            case wCompIdx: return "W";
            case oCompIdx: return "O";
            case gCompIdx: return "G";
        }
    ...
    {
        DUNE_THROW(Dune::NotImplemented, "molarDensity is not implemented for the BlackOil model!");
    }
...
//         Valgrind::CheckDefined(pressure);
    ...
    static Scalar oilSaturationPressure(Scalar X_oG)
    {
        ...
        DUNE_THROW(NumericalProblem, "Could not find the oil saturation pressure for X_o^g = " << X_oG);
    }
