...
#include"dumux/shapefunctions/CRshapefunctions.cc"
                ...
                for (int i=0; i<refelem.size(n); i++)
                    if (it->template subEntity<n>(i)->partitionType()==BorderEntity)
                    {
                        ...
                        for (int j=0; j<refelem.size(n); j++)
                            if (i!=j)
                            {
                                IdType beta = grid.globalIdSet().subId(*it,j,n);
                                myset.insert(beta);
...
//! class for CR finite element functions on a grid
/*! This class implements the interface of a DifferentiableGridFunction
  with piecewise linear elements using a CR basis. It is implemented
  using the general shape functions, thus it should work for all element types
  and dimensions.

  In addition to the DifferentiableGridFunction interface CR functions can be initialized
...
public:
        ...
        : grid_(g), gridview(gv), is(gv.indexSet()), mapper_(gv), lc(lcomm), oldcoeff(0)
    {
        ...
        if (extendoverlap && g.overlapSize(0)>0)
            DUNE_THROW(GridError,"CRFunction: extending overlap requires nonoverlapping grid");
        ...
        try {
            ...
            std::cerr << "not enough memory in CRFunction" << std::endl;
    ...
    */
    virtual RT evallocal (int comp, const Entity& e, const Dune::FieldVector<DT,n>& xi) const
    {
        ...
        for (int i=0; i<Dune::CRShapeFunctions<DT,RT,n>::general(gt,1).size(); ++i)
            value += Dune::CRShapeFunctions<DT,RT,n>::general(gt,1)[i].evaluateFunction(0,xi)*(*coeff)[mapper_.map(e,i,1)][comp];
        return value;
    }
    ...
    */
                                ...
                                const Entity& e, const Dune::FieldVector<DT,n>& xi) const
    {
        ...
        for (int i=0; i<sfs.size(); ++i)
        {
            ...
            value += grad[dir] * (*coeff)[mapper_.map(e,i,1)][comp];
        }
        return value;
    }
...
/** \brief CR finite element function on the leaf grid
...
/** \brief CR finite element function on a given level grid
