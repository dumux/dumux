...
public:
    ...
    , useDirectVelocitySolverForA_(Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.DirectSolverForA", false))
    {
        const bool determineRelaxationFactor = Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.DetermineRelaxationFactor", true);
...
public:
    ...
    virtual void apply(X& update, const Y& currentDefect)
    {
        ...
        for (std::size_t i = 0; i < update.size(); ++i)
        {
            ...
            for (int ii=0; ii < inverseDiagonalElements_[i].size(); ++ii)
            {
                tmp[ii] *= inverseDiagonalElements_[i][ii];
                update[i][ii]= tmp[ii];
            }
        }
    }
...
private:
    ...
    void init_(X& update, const Y& currentDefect)
    {
        ...
        for (std::size_t i = 0; i < inverseDiagonalElements_.size(); ++i)
        {
            ...
            for (int ii = 0; ii < inverseDiagonalElements_[i].size(); ++ii)
            {
                inverseDiagonalElements_[i][ii] = 1.0/res[i][ii];
            }
...
class SeqSimple : public Dune::Preconditioner<X,Y>
{
    ...
    using AType = std::decay_t<decltype(std::declval<M>()[Dune::Indices::_0][Dune::Indices::_0])>;
    using UType = std::decay_t<decltype(std::declval<X>()[Dune::Indices::_0])>;
    using DType = std::decay_t<decltype(std::declval<M>()[Dune::Indices::_1][Dune::Indices::_1])>;
    using VType = std::decay_t<decltype(std::declval<X>()[Dune::Indices::_1])>;
    using BType = std::decay_t<decltype(std::declval<M>()[Dune::Indices::_0][Dune::Indices::_1])>;
    using CType = std::decay_t<decltype(std::declval<M>()[Dune::Indices::_1][Dune::Indices::_0])>;

    using Comm = Dune::Amg::SequentialInformation;
    using LinearOperator = Dune::MatrixAdapter<AType, UType, UType>;
    using Smoother = Dune::SeqSSOR<AType, UType, UType>;
    using AMGSolverForA = Dune::Amg::AMG<LinearOperator, UType, Smoother, Comm>;

    class SimpleOperator : public Dune::LinearOperator<VType, VType>
    {
    public:
         ...
         */
        void apply (const VType& x, VType& y) const final
        {
        ...
        void applyscaleadd (typename VType::field_type alpha, const VType& x, VType& y) const final
        {
            auto tmp = VType(y.size());
            apply(x, tmp);
            y.axpy(alpha, tmp);
        }
    ...
    private:
        ...
        std::function<void(UType&, UType&)> solver_;
    };
...
public:
    ...
    , paramGroup_(params.get<std::string>("ParameterGroup"))
    , useDirectVelocitySolverForA_(Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.DirectSolverForA", false))
    , useDiagonalInSchurComplement_(Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.UseDiagonalInSchurComplement", false))
    , useDiagonalInUpdate_(Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.UseDiagonalInUpdate", false))
    {
        // AMG is needed for determination of omega
        if (!useDirectVelocitySolverForA_)
            initAMG_(params);
        ...
        relaxationFactorP_ = Dumux::getParamFromGroup<scalar_field_type>(paramGroup_, "LinearSolver.Preconditioner.RelaxationP", relaxationFactorP_);
        relaxationFactorU_ = Dumux::getParamFromGroup<scalar_field_type>(paramGroup_, "LinearSolver.Preconditioner.RelaxationU", relaxationFactorU_);
    ...
    virtual void apply(X& update, const Y& currentDefect)
    {
        ...
        static const bool dirichletHack = Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.DirichletHack", false);
        // incorporate Dirichlet cell values
        // TODO: pass Dirichlet constraint handler from outside
        if (dirichletHack)
        for (std::size_t i = 0; i < D.N(); ++i)
...
private:
...
#if HAVE_UMFPACK
            using namespace Dune::Indices;
            umfPackSolverForA_ = std::make_unique<Dune::UMFPack<AType>>(matrix_[_0][_0]);
#else
            DUNE_THROW(Dune::InvalidStateException, "UMFPack not available. Use LinearSolver.Preconditioner.DirectVelocitySolver = false.");
#endif
    }
    ...
    void initSchurComplementSolver_()
    {
        ...
        if (useDiagonalInSchurComplement_)
            linearOperator->setSolver([&](UType& a, UType& b) { applyInverseOfDiagonalOfA_(a); });
        else
            linearOperator->setSolver([&](UType& a, UType& b) { applySolverForA_(a, b); });

        Dune::ParameterTree treeSchurComplementSolver;
        treeSchurComplementSolver["verbose"] = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverVerbosity", "0");
        treeSchurComplementSolver["reduction"] = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverResidualReduction", "1e-12");
        treeSchurComplementSolver["maxit"] = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverMaxIter", "1000");
        treeSchurComplementSolver["restart"] = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverRestart", "100");

        static const bool usePreconditionerForSchurComplement = Dumux::getParamFromGroup<bool>(paramGroup_, "LinearSolver.Preconditioner.UsePreconditionerForSchurComplement", false);
        if (usePreconditionerForSchurComplement)
        {
            Dune::ParameterTree treePreconditioner;
            treePreconditioner["verbose"] = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverVerbosity", "0");
            schurComplementPreconditioner_ = std::make_shared<SeqJacobiMatrixFree<VType, VType>>(linearOperator, treePreconditioner);
        }
        else
            schurComplementPreconditioner_ = std::make_shared<Dune::Richardson<VType, VType>>();

        static const auto schurComplementSolverType = Dumux::getParamFromGroup<std::string>(paramGroup_, "LinearSolver.Preconditioner.SimpleSolverType", "restartedgmressolver");

        if (schurComplementSolverType == "restartedgmressolver")
            schurComplementSolver_ = std::make_unique<Dune::RestartedGMResSolver<VType>>(linearOperator, schurComplementPreconditioner_, treeSchurComplementSolver);
        else if (schurComplementSolverType == "bicgstabsolver")
            schurComplementSolver_ = std::make_unique<Dune::BiCGSTABSolver<VType>>(linearOperator, schurComplementPreconditioner_, treeSchurComplementSolver);
        else if (schurComplementSolverType == "cgsolver")
            schurComplementSolver_ = std::make_unique<Dune::CGSolver<VType>>(linearOperator, schurComplementPreconditioner_, treeSchurComplementSolver);
        else
    ...
    void applyInverseOfDiagonalOfA_(UType& x) const
    {
        using namespace Dune::Indices;
    ...
    std::unique_ptr<AMGSolverForA> amgSolverForA_;
    std::unique_ptr<Dune::IterativeSolver<VType,VType>> schurComplementSolver_;
    std::shared_ptr<Dune::Preconditioner<VType,VType>> schurComplementPreconditioner_;
#if HAVE_UMFPACK
    std::unique_ptr<Dune::UMFPack<AType>> umfPackSolverForA_;
#endif
    const std::string paramGroup_;
    const bool useDirectVelocitySolverForA_;
