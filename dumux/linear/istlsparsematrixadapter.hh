 ...
 */
#ifndef DUMUX_ISTL_SPARSE_MATRIX_ADAPTER_HH
#define DUMUX_ISTL_SPARSE_MATRIX_ADAPTER_HH
 ...
 */
template <class MatrixBlockType, class FvGridGeometry, class AllocatorType=std::allocator<MatrixBlockType> >
...
public:
     ...
     */
    IstlSparseMatrixAdapter(const FvGridGeometry& fvGridGeometry)
        : IstlSparseMatrixAdapter(fvGridGeometry.numDofs(), fvGridGeometry.numDofs())
    {}

    /*!
     * \brief Allocate matrix structure give a sparsity pattern.
     */
    void reserve(const FvGridGeometry& fvGridGeometry)
    {
        istlMatrix_ = std::make_shared<IstlMatrix>();
        istlMatrix_->setBuildMode(IstlMatrix::random);
        // resize the jacobian and the residual
        const auto numDofs = fvGridGeometry.numDofs();
        istlMatrix_->setSize(numDofs, numDofs);

        // create occupation pattern of the jacobian
        const auto occupationPattern = getJacobianPattern<true>(fvGridGeometry);

        // export pattern to jacobian
        occupationPattern.exportIdx(*istlMatrix_);
    }
    ...
    void clearBlock(const size_t row, const size_t col, const Scalar diag = 1.0)
    {
        MatrixBlock diagBlock(Scalar(0));
        for (int i = 0; i < diagBlock.rows; ++i)
            diagBlock[i][i] = diag;

        (*istlMatrix_)[row][col] = diagBlock;
    }

     /*!
     * \brief Fill given block with entries stored in the matrix.
     */
    MatrixBlock& block(const size_t rowIdx, const size_t colIdx)
...
protected:
    ...
    std::shared_ptr<IstlMatrix> istlMatrix_;
};

}} // namespace Linear, Dumux
