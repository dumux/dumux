...
/*!
 * \brief Adapter to turn a BCRS matrix into a thread-parallel linear operator.
 * Adapts a matrix to the assembled linear operator interface
 */
template<class M, class X, class Y>
class ParallelMatrixAdapter : public Dune::MatrixAdapter<M,X,Y>
{
    using ParentType = Dune::MatrixAdapter<M,X,Y>;
public:
    //! export types
    ...
    //! constructor: just store a reference to a matrix
    explicit ParallelMatrixAdapter (const M& A)
    : ParentType(A) {}

    //! constructor: store an std::shared_ptr to a matrix
    explicit ParallelMatrixAdapter (std::shared_ptr<const M> A)
    : ParentType(A) {}
    ...
    void apply (const X& x, Y& y) const override
    {
        ...
        auto& A = this->getmat();
        Dumux::parallelFor(A.N(), [&](const std::size_t ii)
        {
            const auto& row = A[ii];
            const auto endj = row.end();
            for (auto jj=row.begin(); jj!=endj; ++jj)
            {
                const auto& xj = Dune::Impl::asVector(x[jj.index()]);
                auto&& yi = Dune::Impl::asVector(y[ii]);
                Dune::Impl::asMatrix(*jj).umv(xj, yi);
            }
        });
    }
