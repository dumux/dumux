...
public:
    ...
    /*!
     * \brief Returns the dirction index of a unit vector (0 = x, 1 = y, 2 = z)
     */
                                ...
                                const std::string& modelParamGroup)
    {
        ...
        static const bool coupleOverPoreRadius = getParamFromGroup<bool>(modelParamGroup, "Grid.CoupleOverPoreRadius", false);
        if (!coupleOverPoreRadius)
            DUNE_THROW(Dune::GridError, "SnappyGridCreator in 3D only works with Grid.CoupleOverPoreRadius == true");

        if (std::any_of(poreRadius.begin(), poreRadius.end(),
                        [&](auto& r)
                        {
                            using std::abs;
                            return abs(r-poreRadius[0]) > 1e-5*poreRadius[0];
                        }))
        // if (std::any_of(poreRadius.begin(), poreRadius.end(), [&](auto& r) {return r != poreRadius[0];}))
        ...
        for (const auto& element : elements(lowDimGridView))
        {
            for (std::size_t localVertexIdx = 0; localVertexIdx < 2; ++localVertexIdx)
            {
                if (const auto& pos = element.geometry().corner(localVertexIdx); intersectsPointGeometry(pos, line))
                {
                    ...
                    auto throatRadius = [&](const auto& element)
                    {
                        return lowDimGridData.parameters(element)[lowDimGridData.parameterIndex("ThroatInscribedRadius")];
                    };
                    ...
                    static const bool coupleOverPoreRadius = getParamFromGroup<bool>(modelParamGroup, "Grid.CoupleOverPoreRadius", false);
                    const Scalar couplingThroatRadius = coupleOverPoreRadius ? poreRadius
                                                      : projectedThroatRadius(throatRadius(element), element, couplingPlaneNormal);

                    data.push_back({pos[dirIdx], couplingThroatRadius});
                }
            }
        }
...
public:
    ...
    auto init(const OtherGrid& otherGrid, const LowDimGridData& otherData, const std::string& modelParamGroup = "")
    {
        ...
        using SnappyGridCreatorHelper = Dumux::SnappyGridCreatorHelper<typename Grid::LeafGridView>;
        ...
        const auto couplingPlaneNormalDirectionIndex = SnappyGridCreatorHelper::directionIndex(couplingPlaneNormal);
        const auto couplingPlane = SnappyGridCreatorHelper::makeCouplingPlane(lowerLeft, upperRight, couplingPlaneNormal, modelParamGroup);

        std::cout << "plane: \n";
        for (int i = 0; i < couplingPlane.corners(); ++i)
            std::cout << couplingPlane.corner(i) << std::endl;

        const auto pointsOnAxisParallelLines = SnappyGridCreatorHelper::getPointsOnLine(lowerLeft, upperRight, couplingPlaneNormal, otherGrid.leafGridView(), otherData, modelParamGroup_);
        ...
        return GridConstructionData{std::move(positions), std::move(cells), std::move(grading), std::move(interFacePositions)};
    }
