 ...
 */
#ifndef DUMUX_EXPERIMENTAL_FV_ASSEMBLER_HH
#define DUMUX_EXPERIMENTAL_FV_ASSEMBLER_HH
...
public:
    ...
    /*!
        ...
        * \note This interface will change once assembly works with the
    ...
    , gridGeometry_(gridGeometry)
    , timeLoop_(timeLoop)
    , prevGridVars_(&prevGridVariables)
    ...
    /*!
        * \brief Assembles the global Jacobian of the residual and the residual for the current solution.
        */
    template<class PartialReassembler = DefaultPartialReassembler>
    void assembleJacobianAndResidual(GridVariables& gridVars, const PartialReassembler* partialReassembler = nullptr)
    {
        ...
        enforcePeriodicConstraints_(*jacobian_, *residual_, gridVars.dofs(), *gridGeometry_);
    }
...
private:
    ...
    {
        if (!isStationaryProblem_ && !prevGridVars_)
            DUNE_THROW(
                Dune::InvalidStateException,
                "Assembling instationary problem but previous grid variables were not set!"
            );
    }
    ...
    const GridVariables* prevGridVars_ = nullptr;
