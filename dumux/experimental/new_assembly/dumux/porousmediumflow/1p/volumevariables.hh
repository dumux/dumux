 ...
 * \file
 * \ingroup OnePModel
 * \brief Class to encapsulate the quantities required by the single-phase porous medium flow model.
 */
...
#include <dumux/experimental/new_assembly/dumux/porousmediumflow/volumevariables.hh>

namespace Dumux {

template<Concepts::Indexable ElementSolution,
         typename LocalGridGeometry,
         typename LocalEntity>
IndexedType<ElementSolution> interpolate(const ElementSolution& elemSol,
                                         const LocalGridGeometry& localGeom,
                                         const LocalEntity& localEntity)
{
    // TODO: iplement for schemes other than FV somehow?
    return elemSol[localEntity.localDofIndex()];
}
...
{
    static_assert(Concepts::Indexable<typename Traits::PrimaryVariables>);
...
public:
    ...
    template<Concepts::Indexable ElementSolution,
             typename Parameters,
             typename LocalGridGeometry,
             typename LocalEntity> requires(
        std::same_as<IndexedType<ElementSolution>, PrimaryVariables>)
