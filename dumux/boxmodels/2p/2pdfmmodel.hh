...
#ifndef DUMUX_TWOP_DFM_MODEL_HH
#define DUMUX_TWOP_DFM_MODEL_HH
...
//GET_PROP_TYPE(TypeTag, BaseModel)
{
...
public:
	...
	if (numDofs != numVertices)
	  velocityOutput = false;
        ...
        ScalarField *perm = writer.allocateManagedBuffer(numDofs); //TODO change to permeability
        ...
        ScalarField *isFracture = writer.allocateManagedBuffer(numDofs);
        ...
        for (; elemIt != elemEndIt; ++elemIt)
        {
            ...
            if (numDofs == numElements) // element data
            {
                ...
                (*perm)[idx] = volVars.porosity(); //TODO change to permeability
            ...
            else // vertex data
            {
                ...
                for (int i = 0; i < numVerts; ++i)
                {
                    ...
                    (*perm)[globalIdx] = volVars.porosity(); //TODO change to permeability
                    (*poro)[globalIdx] = volVars.porosity();
                    (*Te)[globalIdx] = volVars.temperature();
                    (*isFracture)[globalIdx] = this->problem_().spatialParameters().isVertexFracture(*elemIt, i);
                    if(velocityOutput)
                    {
                        (*cellNum)[globalIdx] += 1;
                    }
        ...
        else // vertex data
        {
            ...
            writer.attachVertexData(*perm, "permeability");//TODO change to permeability
            writer.attachVertexData(*Te, "temperature");
            writer.attachVertexData(*isFracture, "isFracture");
            if(velocityOutput) // check if velocity output is demanded
            {
