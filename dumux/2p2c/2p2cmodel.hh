#ifndef DUNE_TWOPHASETWOCOMPMODEL_HH
#define DUNE_TWOPHASETWOCOMPMODEL_HH
...
template<class G, class RT, class ProblemType, class LocalJacobian,
		class FunctionType, class OperatorAssembler> class TwoPTwoCModel :
...
public:
	...
	TwoPTwoCModel(const G& g, ProblemType& prob) :
		NonlinearModel(g, prob), uOldTimeStep(g) {
	}

	TwoPTwoCModel(const G& g, ProblemType& prob, int level) :
...
class LeafP1TwoPTwoCModel 
: public TwoPTwoCModel<G, RT, ProblemType, LocalJac,
...
public:
	...
	typedef TwoPTwoCModel<G, RT, ProblemType, LocalJac,
	FunctionType, OperatorAssembler> TwoPTwoCModel;

	typedef LeafP1TwoPTwoCModel<G, RT, ProblemType, LocalJac, m> ThisType;
	...
	LeafP1TwoPTwoCModel(const G& g, ProblemType& prob) :
		TwoPTwoCModel(g, prob), problem(prob), grid(g), vertexmapper(g,
