...
template<class Grid, class Scalar, class BoxFunction = LeafP1Function<
                                       Grid, Scalar, 3> >
class Box2P2CNIJacobian: public BoxJacobian<Box2P2CNIJacobian<Grid, Scalar,
...
public:
    ...
    virtual VBlockType computeStorage (const Element& element, const VBlockType* sol, int idx, bool old = false)
    {
        if (old)
    ...
    virtual VBlockType computeFlux (const Element& element, const VBlockType* sol, int face)
    {
        ...
        enthN = 0.5*(vNDat[idx_i].enthalpy[nPhase] + vNDat[idx_j].enthalpy[nPhase]);
        ...
        flux[wComp] += (alpha* vNDat[up_n].density[nPhase]*vNDat[up_n].mobility[nPhase]
            ...
            * outward[nPhase];
        // Nonwetting Component conservation
        ...
        //         // Heat conservation
        //          flux[heat] += diffusionWW*enthW + diffusionAW*enthN; diffusive transport of enthalpy neglected here
     ...
     */
    virtual VBlockType computeSource (const Element& element, const VBlockType* sol, const int& idx)
    {
