 ...
 *        per element (cc schemes) in the context of
 *        instationary problems.
 */
#ifndef DUMUX_CC_INSTATIONARY_LOCAL_ASSEMBLER_HH
#define DUMUX_CC_INSTATIONARY_LOCAL_ASSEMBLER_HH
...
public:
     ...
     * \brief Constructor.
     */
    explicit FVInstationaryLocalAssembler(const Element& element,
                                          const FVElementGeometry& fvGeometry_,
                                          std::vector<ElementVariables>& elemVars,
                                          const StageParams& stageParams)
    : element_(element)
    , fvGeometry_(fvGeometry_)
    ...
    ElementResidualVector evalLocalResidual() const
    {
        ...
        {
            LocalOperator localOperator(element_, fvGeometry_, elemVariables_[k]);
    ...
    {
        const auto eIdxGlobal = fvGeometry_.gridGeometry().elementMapper().index(element_);
        ...
        else
        {
            if constexpr (!isBox)
            {
                ...
                for (int pvIdx = 0; pvIdx < numEq; ++pvIdx)
                    jac[eIdxGlobal][eIdxGlobal][pvIdx][pvIdx] = 1.0;
            }
            else
            {
                ...
                for (const auto& scv : scvs(fvGeometry_))
                {
                    ...
                    const auto& v = element_.template subEntity<dim>(vIdxLocal);

                    // TODO: INCLUDE PARTITION TYPE HEADER
        ...
        auto applyDirichlet = [&] (const auto& scvI,
                                   ...
                                   const auto pvIdx)
        {
            res[scvI.dofIndex()][eqIdx] = elemVariables_.back().elemVolVars()[scvI].priVars()[pvIdx] - dirichletValues[pvIdx];
            ...
            if (fvGeometry_.gridGeometry().dofOnPeriodicBoundary(scvI.dofIndex()))
            {
                ...
                res[periodicDof][eqIdx] = elemVariables_.back().elemVolVars()[scvI].priVars()[pvIdx] - dirichletValues[pvIdx];
    ...
    void evalDirichletBoundaries(ApplyDirichletFunctionType applyDirichlet)
    {
        ...
        {
            // TODO: compatibility with cc? Use ElementBoundaryTypes again???
            const auto bcTypes = problem_().boundaryTypes(element_, scvI);
            if (bcTypes.hasDirichlet())
            {
                const auto dirichletValues = problem_().dirichlet(element_, scvI);
...
protected:
    ...
    /*!
     * \brief Computes the derivatives with respect to the given element and
     *        adds them to the global matrix.
     * \return The element residual at the current solution.
     * \note This specialization is for the box scheme with numeric differentiation
     */
    template< class PartialReassembler = DefaultPartialReassembler,
              int b = isBox, DiffMethod dm = diffMethod,
              std::enable_if_t<(b && dm == DiffMethod::numeric), int> = 0 >
                                                       ...
                                                       const PartialReassembler* partialReassembler = nullptr)
    {
        ...
        auto& curVariables = elemVariables_.back();
        ...
        const auto origElemSol = elementSolution(element_, x, fvGeometry_.gridGeometry());
        ...
        for (const auto& scvI : scvs(fvGeometry_))
        {
            ...
            for (int pvIdx = 0; pvIdx < numEq; pvIdx++)
            {
                ...
                auto evalResiduals = [&](Scalar priVar)
                {
                    ...
                    curVolVars.update(elemSol, problem_(), element_, scvI);
