...
public:
     ...
     */
    template <class ResidualVector, class PartialReassembler = DefaultPartialReassembler, class CouplingFunction = Detail::NoOperator>
    ...
    void evalDirichletBoundaries(ApplyDirichletFunctionType applyDirichlet)
    {
        ...
        if (this->elemBcTypes().hasDirichlet())
        {
            for (const auto& scvf : scvfs(this->fvGeometry()))
            {
                if (scvf.boundary())
                {
                    const auto bcTypes = this->elemBcTypes().get(this->fvGeometry(), scvf);
...
public:
                                                          ...
                                                          const PartialReassembler* partialReassembler = nullptr)
    {
        ...
        static const bool updateAllVolVars = getParamFromGroup<bool>(
            problem.paramGroup(), "Assembly.FCDiamondVolVarsDependOnAllElementDofs", false
        );
