        ...
        if (this->elemBcTypes().hasDirichlet())
        {
            for (const auto& scvf : scvfs(this->fvGeometry()))
            {
                if (scvf.isFrontal() && scvf.boundary())
                {
                    ...
                    {
                        if (scvf.directionIndex() == 0 && scvf.directionSign() > 0 && scvf.ipGlobal()[1] > 2.1)
                            std::cout << "ffuffu" << std::endl;
                        ...
                        // const auto dirichletValues = this->problem().dirichlet(this->element(), scvf);
