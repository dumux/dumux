 ...
 * \param notAssigned (possibly non-zero capacity) bool container (passed from outside for memory efficiency)
 */
int smallestAvailableColor(const std::vector<int>& colors,
                           std::vector<bool>& notAssigned)
{
    const int numColors = colors.size();
    notAssigned.assign(numColors, true);
...
auto coloredElementSets(const GridGeometry& gg, int verbosity = 1)
{
    ...
    std::vector<bool> notAssigned; notAssigned.reserve(30);
