...
namespace Dune
{
  ...
  */
  ...
  class BoxSwJacobian
    : public BoxJacobian<BoxSwJacobian<Grid,Scalar,BoxFunction>,Grid,Scalar,1,BoxFunction>
  {
    typedef typename Grid::Traits::template Codim<0>::Entity Entity;
    typedef typename Entity::Geometry Geometry;
    typedef BoxSwJacobian<Grid,Scalar,BoxFunction> ThisType;
  ...
  public:
	    ...
	    typedef FieldVector<Scalar,dim> FVector;
	     ...
	     //! Constructor
    BoxSwJacobian (RichardsProblem<Grid,Scalar>& params,
...
//		varData[i].K = problem.K(this->fvGeom.subContVol[i].global, e, this->fvGeom.subContVol[i].local);
		varData[i].satW = sol[i][satWIdx];
   		...
   		varData[i].pW = -varData[i].pC;
   		varData[i].mobility[satWIdx] = problem.materialLaw().mobW(varData[i].satW, global, e, local);
   		varData[i].density[satWIdx] = problem.wettingPhase().density(283.15, varData[i].pW);
    }
