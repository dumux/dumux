...
namespace Dune
{
  ...
  */
  ...
  class BoxSwJacobian
    : public BoxJacobian<BoxSwJacobian<G,RT,BoxFunction>,G,RT,1,BoxFunction>
  {
    typedef typename G::ctype DT;
    typedef typename G::Traits::template Codim<0>::Entity Entity;
    typedef typename Entity::Geometry Geometry;
    typedef BoxSwJacobian<G,RT,BoxFunction> ThisType;
...
//		varData[i].K = problem.K(this->fvGeom.subContVol[i].global, e, this->fvGeom.subContVol[i].local);
		varData[i].satW = sol[i][satWIdx];
   		...
   		varData[i].mobility[satWIdx] = problem.materialLaw().mobW(varData[i].satW, global, e, local, problem.temperature());
   		varData[i].density[satWIdx] = problem.wettingPhase().density(problem.temperature(), varData[i].pW + problem.pN());
    }
