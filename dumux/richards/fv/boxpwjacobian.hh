...
public:
                            ...
                            int i, std::vector<VariableNodeData>& varData)
    {
        ...
        varData[i].mobility[pWIdx] = problem.materialLaw().mobW(varData[i].satW, global, e, local, problem.temperature());
        varData[i].density[pWIdx] = problem.wettingPhase().density(problem.temperature(), varData[i].pW + problem.pN());
    }
