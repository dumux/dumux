 ...
 */
#ifndef DUMUX_PHASEFIELD_ALLENCAHN_FE_LOCAL_RESIDUAL_HH
#define DUMUX_PHASEFIELD_ALLENCAHN_FE_LOCAL_RESIDUAL_HH
 ...
 *        Allen-Cahn local residual (non-control-volume local dofs).
 */
template<class NumEqVector, class Indices>
class AllenCahnFELocalResidualTerms
{
public:
    /*!
                                      ...
                                      const ElementVariables& elemVars)
    {
        if constexpr (Detail::LocalDofs::hasNonCVLocalDofsInterface<FVElementGeometry>())
        {
            ...
            // The double-well reaction term f'(phi) is cubic in phi (phi
            // itself degree-2 on PQ2), so as a function of position it is
            // degree 6; combined with a degree-2 test function this needs an
            // exact degree-8 rule -- the test's properties.hh raises the grid
            ...
            for (const auto& qpData : CVFE::quadratureRule(fvGeometry, element))
            {
                ...
                for (const auto& localDof : nonCVLocalDofs(fvGeometry))
                {
                    ...
                    // weak-form diffusive term: +grad(N_i) . (M gamma grad(phi))
                    fluxAndSourceTerm[Indices::phaseFieldEqIdx] +=
                        problem.mobility()*problem.surfaceTension()*(context.gradPhaseField()*ipCache.gradN(localDofIdx));
