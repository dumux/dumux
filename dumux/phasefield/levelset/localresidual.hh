...
/*!
 * \file
 * \ingroup LevelSetModel
 * \brief Element-wise calculation of the local residual for the conservative
 *        level-set model using hybrid control-volume/finite-element (CVFE)
 *        discretizations.
 */
#ifndef DUMUX_PHASEFIELD_CONSERVATIVE_LEVEL_SET_LOCAL_RESIDUAL_HH
#define DUMUX_PHASEFIELD_CONSERVATIVE_LEVEL_SET_LOCAL_RESIDUAL_HH
 ...
 * \brief Element-wise calculation of the conservative phase-field level-set local
 *        residual for hybrid CVFE discretizations.
 *
 ...
 * `PhaseFieldLevelSetFELocalResidualTerms` (see
 * `addToElementStorageResidual`/`addToElementFluxAndSourceResidual`).
 *
 * There is no algebraic-identity closure equation (unlike Cahn-Hilliard), so the
 * `sourceIntegral` provided by the base class (calling `problem.source(...)`
 * directly) is used unmodified; the reinitialization equation itself has no
 * source term unless the problem adds one.
 */
template<class TypeTag>
class PhaseFieldLevelSetLocalResidual
: public DiscretizationDefaultLocalOperator<TypeTag>
{
    ...
    using FeResidual = LevelSetFELocalResidualTerms<NumEqVector, Indices>;
...
public:
     ...
     *
     * The flux is the sum of the (nonlinear) compression term
     * \f$ \phi(1-\phi)\mathbf{n} \f$, which sharpens the interface profile,
     * and a standard diffusive term \f$ -\varepsilon \nabla\phi \f$, which
     * smooths it; together they maintain a fixed-width profile without
     * moving the interface location.
     *
     * \param fvGeometry The finite-volume geometry of the element
     * \param elemVars The variables for all local dofs of the element
                             ...
                             const SubControlVolumeFace& scvf) const
    {
        ...
        for (const auto& qpData : CVFE::quadratureRule(fvGeometry, scvf))
        {
            ...
            const auto& faceNormal = ipData.unitOuterNormal();

            NumEqVector fluxAtIp(0.0);
            fluxAtIp[Indices::phaseFieldEqIdx] =
                compressionCoeff*(normal*faceNormal) - problem.epsilon()*(context.gradPhaseField()*faceNormal);
