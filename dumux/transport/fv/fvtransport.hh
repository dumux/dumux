...
namespace Dune
{
  ...
  public:
	   ...
	   */
	   ...
	   void vtkout (const char* name, int k) const 
	   {
		 ...
		 */
		FVTransport(G& g, TransportProblem<G, RT, VelType>& prob, int lev = 0,
				   	  DiffusivePart<G, RT>& diffPart = *(new DiffusivePart<G, RT>),
				   	  bool rec = false, double amax = 0.8, double cFLFac = 1.0, 
		...
		: Transport<G, RT, RepresentationType, VelType>(g, prob,lev), 
		  ...
		  numFlux(numFl), diffusivePart(diffPart), alphamax(amax), cFLFactor(cFLFac)
		{
			this->sat.resize(elementmapper.size());
			this->problem.velocity.resize(elementmapper.size());
		}
  ...
  template<class G, class RT>
	int FVTransport<G,RT>::update(const RT t, RT& dt, RepresentationType& updateVec) 
	{
...
// yufei temmp
			...
			// get permeability value at cell center
//			double permI = perm[indexi];
...
//					    if (satGradient*integrationOuterNormal < 0)
//					    	satAvg = satI;
//					    else 
//					    	satAvg = satJ;
					    ...
					    double fI = this->problem.materialLaw.fractionalW(satI);
					    double fJ = this->problem.materialLaw.fractionalW(satJ);
				...
				if (is->boundary()) 
				{
			    	...
			    	double fI = this->problem.materialLaw.fractionalW(satI);
			    	double fBound = this->problem.materialLaw.fractionalW(satBound);
					diffFactor = diffPart / volume;
			    	factor = diffFactor
  ...
  void FVTransport<G,RT>::initial() 
  	{
  	    ...
  	    for (Iterator it = indexset.template begin<0,All_Partition>(); it != eendit; ++it)
  		{
  			...
  			this->sat[elementmapper.map(*it)] = this->problem.S0(global, *it, local);
  		}
  ...
  template<class G, class RT>
  void FVTransport<G,RT>::CalculateSlopes(SlopeType& slope, RT t)
  	  {
  	    ...
  	    for (Iterator it = this->grid.template lbegin<0>(this->level()); it!=endit; ++it)
  	  	{
  	  	  ...
  	  	  for (IntersectionIterator is = IntersectionIteratorGetter<G,LevelTag>::begin(*it); is!=isend; ++is)
  	  	    ...
  	  	    for (int k = 0; k < dim; k++) {
  	  	    	...
  	  	    	{
  	  	    		double satI = this->sat[indexi];
  	  	    		double alphaRight = stabilityFactor*fabs(2.0/(dist[location[2*k]]*slopeIK)*(saturation[location[2*k]] - satI));
  	  	    		double alphaLeft = stabilityFactor*fabs(2.0/(dist[location[2*k + 1]]*slopeIK)*(satI - saturation[location[2*k + 1]]));
