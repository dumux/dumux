...
public:
	 ...
	 */
    ChTransport(G& g, TransportProblem<G, RT, VC>& prob,
...
private:
 ...
 RT ChTransport<G,RT,VC>::linearflux(RT sat)
  ...
  int ChTransport<G,RT,VC>::init(const RT dt,const RT lambda, EntityPointer& it)
  {
    ...
    GV& gridView = g.levelView(this->level());
    ...
    IntersectionIterator endis = gridView.template iend(*it);
    for(IntersectionIterator is=gridView.template ibegin(*it);is!=endis;++is)
    {
