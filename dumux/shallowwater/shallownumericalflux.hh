// $Id: numericalflux.hh 566 2008-09-11 11:38:31Z bernd $ 
...
namespace Dune
{
template<class G, class DT> class NumericalFlux
{
public:
	...
	virtual SystemType operator()(VelType velFaceL, VelType velFaceR, DT wDepthFaceL,
			DT wDepthFaceR, VelType nVecScaled, VelType nVec)=0;
...
template<class G, class DT> class FirstOrderUpwind : public NumericalFlux<G,DT>
{
	enum
	{	dim=G::dimension};
	typedef Dune::FieldVector<DT, dim> VelType;
	typedef Dune::FieldVector<DT, dim+1> SystemType; //System has 3 euqations -> 3 positions in a vector
	typedef typename G::Traits::template Codim<0>::Entity Entity;
	DT avgVel;
	DT faceVel;
	DT faceWDepth;
...
public:
	...
	SystemType operator()(VelType velI, VelType velJ, DT wDepthI,
			DT wDepthJ, VelType nVecScaled, VelType nVec)
	{
		...
		conti = velFace * wDepthFace;
		xMomentum = velFace * wDepthFace*velFace;
		xMomentum += gravity*wDepthFace*0.5;
		
		DT fluxConti = conti;
		DT fluxXMomentum = xMomentum;
...
template<class G, class DT> class HllFlux : public NumericalFlux<G,DT>
{
	enum
	{	dim=G::dimension};
	typedef Dune::FieldVector<DT, dim> VelType;
...
public:
	...
	HllFlux() :
		NumericalFlux<G, DT>(), gravity(9.81), eps(1e-8)
	{
	...
	SystemType operator()(VelType velFaceL, VelType velFaceR, DT wDepthFaceL,
			DT wDepthFaceR, VelType nVecScaled, VelType nVec)
	{
		...
		else
		{
			...
			DT velAvg=(velFaceR+velFaceL)*nVec;
			velAvg*=0.5;
			...
			DT help1 = (velFaceR+velFaceL)*nVec;
			help1 *=0.25;
			DT help2 = sqrt(gravity*wDepthFaceR)+sqrt(gravity*wDepthFaceL);
			help2 *=0.5;
			DT gravWDepthAvg=(help1+help2);
