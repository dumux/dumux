 ...
 * \brief Stencil-local finite volume geometry (scvs and scvfs) for cell-centered Godunov models
 *        This builds up the sub control volumes and sub control volume faces
 *        for each element in the local scope we are restricting to, e.g. stencil or element.
 */
#ifndef DUMUX_DISCRETIZATION_GODUNOV_FV_ELEMENT_GEOMETRY_HH
#define DUMUX_DISCRETIZATION_GODUNOV_FV_ELEMENT_GEOMETRY_HH
...
class GodunovFVElementGeometry;
 ...
 */
template<class GG>
class GodunovFVElementGeometry<GG, true>
{
    using ThisType = GodunovFVElementGeometry<GG, true>;
...
public:
    ...
    //! Constructor
    GodunovFVElementGeometry(const FVGridGeometry& fvGridGeometry)
    ...
    scvs(const GodunovFVElementGeometry& fvGeometry)
    {
    ...
    scvfs(const GodunovFVElementGeometry& fvGeometry)
    {
 ...
 */
template<class GG>
class GodunovFVElementGeometry<GG, false>
{
    using ThisType = GodunovFVElementGeometry<GG, false>;
    using GridView = typename GG::GridView;
    using IndexType = typename GridView::IndexSet::IndexType;
    using Element = typename GridView::template Codim<0>::Entity;
