 ...
 * \brief The finite volume geometry (scvs and scvfs) for cell-centered Godunov models on a grid view
 *        This builds up the sub control volumes and sub control volume faces
 *        for each element of the grid partition.
 */
#ifndef DUMUX_DISCRETIZATION_GODUNOV_FV_GRID_GEOMETRY_HH
#define DUMUX_DISCRETIZATION_GODUNOV_FV_GRID_GEOMETRY_HH
...
#include <dumux/discretization/cellcentered/godunov/fvelementgeometry.hh>
...
/*!
 * \ingroup GodunovDiscretization
 * \brief The default traits for the Godunov finite volume grid geometry
...
struct GodunovDefaultGridGeometryTraits
: public MapperTraits
{
    using SubControlVolume = CCSubControlVolume<GridView>;
    using SubControlVolumeFace = GodunovSubControlVolumeFace<GridView>;

    template<class FVGridGeometry>
    using ConnectivityMap = CCSimpleConnectivityMap<FVGridGeometry>;

    template<class FVGridGeometry, bool enableCache>
    using LocalView = GodunovFVElementGeometry<FVGridGeometry, enableCache>;
...
template<class GridView,
         ...
         class Traits = GodunovDefaultGridGeometryTraits<GridView> >
class GodunovFVGridGeometry;
...
class GodunovFVGridGeometry<GV, true, Traits>
: public BaseFVGridGeometry<GodunovFVGridGeometry<GV, true, Traits>, GV, Traits>
{
    using ThisType = GodunovFVGridGeometry<GV, true, Traits>;
...
public:
    ...
    static constexpr DiscretizationMethod discMethod = DiscretizationMethod::godunov;
    ...
    //! Constructor
    GodunovFVGridGeometry(const GridView& gridView)
    : ParentType(gridView)
    {
        // Check if the overlap size is what we expect
        if (!CheckOverlapSize<DiscretizationMethod::godunov>::isValid(gridView))
            DUNE_THROW(Dune::InvalidStateException, "The godunov discretization method needs at least an overlap of 1 for parallel computations. "
 ...
 */
template<class GV, class Traits>
class GodunovFVGridGeometry<GV, false, Traits>
: public BaseFVGridGeometry<GodunovFVGridGeometry<GV, false, Traits>, GV, Traits>
{
    using ThisType = GodunovFVGridGeometry<GV, false, Traits>;
    using ParentType = BaseFVGridGeometry<ThisType, GV, Traits>;
    using ConnectivityMap = typename Traits::template ConnectivityMap<ThisType>;
