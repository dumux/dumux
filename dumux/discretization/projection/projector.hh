 ...
 *        functions of its element pair exactly on affine geometries, twice the higher of
 *        the two basis orders.
                              ...
                              std::optional<int> quadratureOrder = {})
{
    ...
    using std::max;
    const auto integrationOrder = [&] (const auto& is, unsigned int targetOrder)
    {
        if (quadratureOrder)
        ...
        {
            domainLocalView.bind( is.domainEntity(nIdx) );
            order = max(order, domainLocalView.tree().finiteElement().localBasis().order());
        }
        return static_cast<int>(max(2*order, 1u));
    };
    ...
    for (const auto& is : intersections(glue))
    {
        ...
        const int intOrder = integrationOrder(is, targetLocalBasis.order());
