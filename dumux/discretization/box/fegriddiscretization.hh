 ...
 * \brief Base class for the finite element grid discretization for the PQ1 (box) FE method.
 *        Treats all vertex degrees of freedom as pure FE dofs (no CV splitting).
 *        Analogous to PQ2FEGridDiscretization for the quadratic case.
 */
...
#include <dumux/discretization/box/feelementdiscretization.hh>
#include <dumux/discretization/extrusion.hh>
#include <dumux/discretization/cvfe/quadraturerules.hh>

#include <dumux/discretization/pq2/geometryhelper.hh> // FEPQ2GeometryHelper reused for P1
...
/*!
 * \ingroup BoxDiscretization
 * \brief The default traits for the PQ1 FE grid discretization
...
: public MapperTraits, public QuadratureTraits
{
    ...
    using LocalView = PQ1FEElementDiscretization<GridDiscretization, enableCache>;

    // maximum number of dofs per element: 2^dim for cubes (same as box)
 ...
 *        Uses linear Lagrange elements with vertex-only dofs.
 *        Analogous to PQ2FEGridDiscretization.
 */
template<class Scalar,
         class GV,
         bool enableCaching = true,
         class Traits = PQ1FEDefaultGridDiscretizationTraits<GV>>
class PQ1FEGridDiscretization
: public BaseGridGeometry<GV, Traits>
{
    using ThisType = PQ1FEGridDiscretization<Scalar, GV, enableCaching, Traits>;
    ...
    static_assert(dim > 0, "Only implemented for dim > 0");

public:
    //! export the discretization method this geometry belongs to (Box = CVFE<PQ1>)
    ...
    //! export the finite element cache type (P1 Lagrange)
    ...
    PQ1FEGridDiscretization(std::shared_ptr<BasicGridGeometry> gg)
    ...
    //! Constructor
    PQ1FEGridDiscretization(const GridView& gridView)
    : PQ1FEGridDiscretization(std::make_shared<BasicGridGeometry>(gridView))
    {}

    //! The dofMapper (vertex mapper)
    const DofMapper& dofMapper() const
    { return dofMapper_; }

    //! The total number of degrees of freedom
    std::size_t numDofs() const
    { return dofMapper_.size(); }
    ...
    friend inline LocalView localView(const PQ1FEGridDiscretization& gg)
    { return { gg.cache_ }; }

private:
    class PQ1FEGridDiscretizationCache
    {
        friend class PQ1FEGridDiscretization;
    public:
        //! export the geometry helper type (reuse FEPQ2GeometryHelper — fully generic)
        using GeometryHelper = FEPQ2GeometryHelper<GV>;

        explicit PQ1FEGridDiscretizationCache(const PQ1FEGridDiscretization& gg)
        : gridDiscretization_(&gg)
        {}

        const PQ1FEGridDiscretization& gridDiscretization() const
...
private:
    ...
    void update_()
    {
        ...
        // error check: periodic boundaries currently don't work for pq1fe in parallel
        if (this->isPeriodic() && this->gridView().comm().size() > 1)
            DUNE_THROW(Dune::NotImplemented, "Periodic boundaries for PQ1 FE method for parallel simulations!");
    }
