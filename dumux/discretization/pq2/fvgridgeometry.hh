...
using PQ2QuadratureTraits = CVFE::DefaultQuadratureTraits<GridView, ScvRule, ScvfRule, ElementRule, IntersectionRule, BoundaryFaceRule>;
            ...
            for (LocalIndexType keyIdx = 0; keyIdx < localCoefficients.size(); ++keyIdx)
            {
                ...
                if(localKey.codim() == dim)
                {
                    ...
                    cache_.scvs_[eIdx][localIdx] = SubControlVolume(
                        ...
                        keyIdx,
                        eIdx,
                        DofHelper::dofIndex(this->dofMapper(), element, localKey),
                        false
                    );
                }
            }
            ...
            for (const auto& intersection : intersections(this->gridView(), element))
            {
                if (intersection.boundary() && !intersection.neighbor())
                {
                    ...
                    for (LocalIndexType keyIdx = 0; keyIdx < localCoefficients.size(); ++keyIdx)
                    {
                        ...
                        {
                            const auto dofIdxGlobal = DofHelper::dofIndex(this->dofMapper(), element, localCoefficients.localKey(keyIdx));
                ...
                else if (periodicGridTraits_.isPeriodic(intersection))
                {
                    ...
                    for (int localDofIdx = 0; localDofIdx < localCoefficients.size(); ++localDofIdx)
                    {
                        ...
                        const auto dofIdxGlobal = DofHelper::dofIndex(this->dofMapper(), element, localCoefficients.localKey(localDofIdx));
