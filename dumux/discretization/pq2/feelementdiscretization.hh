 ...
 * \brief Base class for the local finite element discretization for the pq2 method
 */
#ifndef DUMUX_DISCRETIZATION_PQ2_FE_ELEMENT_DISCRETIZATION_HH
#define DUMUX_DISCRETIZATION_PQ2_FE_ELEMENT_DISCRETIZATION_HH
...
/*!
 * \ingroup PQ2Discretization
 * \brief Base class for the finite element discretization for pq2 discretizations.
 *        All degrees of freedom (vertices, edges, face centers, element center) are
 *        treated as FE dofs (no splitting into CV and non-CV dofs).
...
class PQ2FEElementDiscretization
{
...
public:
    ...
    //! the maximum number of dofs per element (for cubes)
    static constexpr std::size_t maxNumElementDofs = GridDiscretization::maxNumElementDofs;

    //! Constructor
    PQ2FEElementDiscretization(const GGCache& ggCache)
    : ggCache_(&ggCache)
    {}

    //! iterate over all local dofs (all shape function dofs: vertices, edges, face/cell centers)
    friend inline auto localDofs(const PQ2FEElementDiscretization& elemDisc)
    {
    ...
    //! iterate over non-vertex (non-CV) local dofs (edge, face, element-center dofs)
    friend inline auto nonCVLocalDofs(const PQ2FEElementDiscretization& elemDisc)
    {
        return localDofs(elemDisc);
    }

    //! an iterator over all local dofs related to a boundary face
    friend inline auto localDofs(const PQ2FEElementDiscretization& elemDisc, const BoundaryFace& boundaryFace)
    {
    ...
    boundaryFaces(const PQ2FEElementDiscretization& elemDisc)
    {
     ...
     */
    PQ2FEElementDiscretization bind(const Element& element) &&
    {
     ...
     */
    PQ2FEElementDiscretization bindElement(const Element& element) &&
    {
        this->bindElement(element);
        return std::move(*this);
    }
    ...
    friend inline auto ipData(const PQ2FEElementDiscretization& elemDisc, const LocalDof& localDof)
    {
    ...
    friend inline auto ipData(const PQ2FEElementDiscretization& elemDisc,
