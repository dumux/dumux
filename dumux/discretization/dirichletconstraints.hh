...
public:
     ...
     * \param dirichlet Function for evaluating Dirichlet values
     */
    template<class Problem, typename DirichletFunction>
    void update(const Problem& problem, const DirichletFunction& dirichlet)
    {
        ...
        for (const auto& element : elements(problem.gridGeometry().gridView()))
        {
            ...
            for (const auto& intersection : intersections(problem.gridGeometry().gridView(), element))
            {
                ...
                if (bcTypes.hasDirichlet())
                {
                    ...
                    {
                        const auto dirichletValues = dirichlet(fvGeometry, intersection, localDof);
                        ConstraintInfo info;
