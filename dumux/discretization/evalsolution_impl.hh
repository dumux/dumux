 ...
 * \brief Implementation details of the `evalSolution` functions.
 */
#ifndef DUMUX_DISCRETIZATION_EVAL_SOLUTION_IMPL_HH
#define DUMUX_DISCRETIZATION_EVAL_SOLUTION_IMPL_HH
...
PrimaryVariables evalIgnoringState(const Element& element,
...
auto indexOfMinDistVertex(const Geometry& geometry,
 ...
 */
template<class Element, class FVElementGeometry, class PrimaryVariables>
auto evalSolutionImpl(const Element& element,
                      const typename Element::Geometry& geometry,
                      const typename FVElementGeometry::FVGridGeometry& fvGridGeometry,
                      const BoxElementSolution<FVElementGeometry, PrimaryVariables>& elemSol,
                      const typename Element::Geometry::GlobalCoordinate& globalPos,
...
{
    return evalIgnoringState(element, geometry, fvGridGeometry, elemSol, globalPos);
}
...
-> typename std::enable_if_t<decltype(isValid(Detail::hasState())(elemSol[0]))::value, PrimaryVariables>
{
    ...
    if (allStatesEqual)
    {
        auto result = evalIgnoringState(element, geometry, fvGridGeometry, elemSol, globalPos);
    ...
    else
    {
        ...
        auto minDistanceIdx = indexOfMinDistVertex(geometry, globalPos);
...
-> typename std::enable_if_t<!decltype(isValid(Detail::hasState())(elemSol[0]))::value, PrimaryVariables>
{
    return evalIgnoringState(element, geometry, elemSol, globalPos);
}
...
-> typename std::enable_if_t<decltype(isValid(Detail::hasState())(elemSol[0]))::value, PrimaryVariables>
{
    ...
    if (allStatesEqual)
    {
        auto result = evalIgnoringState(element, geometry, elemSol, globalPos);
