...
inline static auto normalAxis(Vector&& v)
{
    using std::abs;
    static constexpr auto eps = 1e-8;
    assert(std::any_of(v.begin(), v.end(), [eps = eps](const auto& x) { return abs(x) > eps; }));

    const auto result = std::find_if(v.begin(), v.end(), [eps = eps](const auto& x) { return abs(x) > eps; } ) - v.begin();
