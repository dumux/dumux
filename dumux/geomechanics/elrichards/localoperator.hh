 ...
 * elrichardslocalresidual (box discretized mass balances)
 * and alphaMomentum (FE discretized momentum balance).
 */
#ifndef DUMUX_ELRICHARDS_LOCAL_OPERATOR_HH
#define DUMUX_ELRICHARDS_LOCAL_OPERATOR_HH
...
//#include <dumux/material/fluidmatrixinteractions/2pDP/vangenuchtenparams.hh>//khodam
...
class ElRichardsLocalOperator
    :
    public Dune::PDELab::FullVolumePattern,
    public Dune::PDELab::LocalOperatorDefaultFlags
{
    // copying the local operator for PDELab is not a good idea
    ElRichardsLocalOperator(const ElRichardsLocalOperator &);
...
public:
     ...
     */
    ElRichardsLocalOperator(Model &model)
        : model_(model)
    {}
                       ...
                       const LFSV& lfsv, R& r) const
    {
        ...
        typedef typename LFSU::template Child<0>::Type PressLFS;
        const PressLFS& pressLFS = lfsu.template child<0>();

        unsigned int numScv = eg.entity().subEntities(dim);

        for (size_type i = 0; i < numScv; i++)
        {
                    ...
                    const LFSV& lfsv, R& r) const
    {
        ...
        Scalar lambda = lameParams[0]; //elasticity
        ...
        for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
        {
            ...
            for(int i = 0; i < dim; ++i)
             {
                effStress[i][i] = lambda*traceEpsilon;// this lambda is related to elasticity and has nothing to do with brookscorey lambda!!!S
                for(int j = 0; j < dim; ++j)
          ...
          //params.vgn();params.vgAlpha();params.vgm();

             Scalar minPc = MaterialLaw::pc(materialParams, 1.0);

             Scalar pnRef = 1.0e5; //0;

             Scalar pweff = pw + minPc;

             pn = std::max(pnRef, pweff);
             Scalar sw = MaterialLaw::sw(materialParams, pn - pw);

             RT_P sn = 1.0 - sw;

//              Scalar pc = pn-pw;
//              pc = std::max(pc, 0.0); //pc [pa]
//              Scalar sw = MaterialLaw::sw(materialParams, pc);
//              RT_P sn = 1.0 - sw;
             RT_P pEff;//change it due to vangenuchten!!!!!!!!!!!!!
...
//                 model_.problem().neumannAtPos(traction, globalPos);
                 model_.problem().solDependentNeumann(traction, eg.entity(), fvGeometry, intersection, 0, fIdx, elemVolVars);
