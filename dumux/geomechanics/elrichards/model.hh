...
* \file
*
* \brief Adaption of the fully implicit scheme to the Richards linear elasticity model.
*/

#ifndef DUMUX_ELASTICRICHARDS_MODEL_HH
#define DUMUX_ELASTICRICHARDS_MODEL_HH
 ...
 * \brief Adaption of the fully implicit scheme to the Richards linear elasticity model.
 *
 * This model implements a Richards flow of compressible immiscible fluids \f$\alpha \in \{ w, n \}\f$.
...
class ElRichardsModel: public GET_PROP_TYPE(TypeTag, BaseModel)
{
...
//         ScalarField &SuctionStress = *writer.allocateManagedBuffer(numVertices);//khodam
...
//         ScalarField &suctionStress = *writer.allocateManagedBuffer(numElements);//khodam
        ...
        ScalarField &theta = *writer.allocateManagedBuffer(numElements);
        ScalarField &S0 = *writer.allocateManagedBuffer(numElements);//khodam

        ScalarField &Pcrtens = *writer.allocateManagedBuffer(numElements);
        ScalarField &Pcrshe = *writer.allocateManagedBuffer(numElements);
        ScalarField &E = *writer.allocateManagedBuffer(numElements);//khodam
        ScalarField &B = *writer.allocateManagedBuffer(numElements);//khodam
        ScalarField &nu = *writer.allocateManagedBuffer(numElements);//khodam
...
//             suctionStress[eIdx] = Scalar(0.0);//khodam
        ...
        for (const auto& element : elements(this->gridView_())) {
            ...
            //theta[eIdx] = this->problem().spatialParams().theta(element, fvGeometry);
            //S0[eIdx] = this->problem().spatialParams().S0(element, fvGeometry);

            // loop over all local vertices of the cell
            int numScv = element.subEntities(dim);
            //subcontrol volume. friction angle and cohesion doesnt need to be within this loop and makes it inefficient
            for (int scvIdx = 0; scvIdx < numScv; ++scvIdx)
            {
                unsigned int vIdxGlobal = this->dofMapper().subIndex(element, scvIdx, dim);

                Te[vIdxGlobal] = elemVolVars[scvIdx].temperature();
                pw[vIdxGlobal] = elemVolVars[scvIdx].pressure(wPhaseIdx);//-1.e5; khodam
...
//                else
                ...
                Keff =    this->problem().spatialParams().intrinsicPermeability(    element, fvGeometry, scvIdx);//[0][0]; we add the rest when K_ is a tensor in spacialparams, but now K_ is a scalar
                Keff *= exp(exponent);
                ...
                suction[eIdx] += (1.e5 -  (pw[vIdxGlobal]))/ numScv;//khodam
                suctionStress[eIdx] += -sw[vIdxGlobal] * (1.e5 -  (pw[vIdxGlobal]))/ numScv;//khodam
//                 std::cout<< "waterContent="<<waterContent[eIdx] << std::endl;
            ...
            //Khodam: three following lines are copied from model of decoupled
            ...
            if(rockMechanicsSignConvention_){
                totalEffStressX[eIdx][0] = initStressX[eIdx][0] + deltaEffStressX[eIdx][0] -  initialPressure[eIdx] - 0.25 * 1.e5;//air and water pressure is the same in all directions therefore the total initial pressure is considered in x direction
                if (dim >= 2) {
...
//             Scalar suctionStress = suctionStress[eIdx];//khodam

//this has been defined for a 1 layer soil. in case of having a soil with different layers we should make these parameteres conditional!!!!but we cannot define a function within the current function and because it was more than 1 value, I defined it in spatialparams
//             Scalar theta = M_PI / 6;
//             Scalar theta = GET_RUNTIME_PARAM(TypeTag, Scalar, FailureParameters.FrictionAngle)*M_PI/180;//friction angle radian
//             std::cout<< "theta(friction angle))="<< theta  <<std::endl;
//             std::cout<< "tan(theta(friction angle))="<< tan(theta)  <<std::endl;
//             std::cout<< "cos(theta(friction angle))="<< cos(theta)  <<std::endl;
//             Scalar S0 = GET_RUNTIME_PARAM(TypeTag, Scalar, FailureParameters.Cohesion);//Cohesion. before it was a fixed number. in order to avoid compilation for each time I did in input file and read it from there
//             std::cout<< "S0(cohesion)="<< S0  <<std::endl;
...
//             sigmamI[eIdx] = (principalStress3[eIdx] + principalStress1[eIdx]) / 2.0 - Peff;//khodam
            sigmamI[eIdx] = abs(sigmam[eIdx]) - suctionStress[eIdx];//khodam after Greco paper
//             sigmamI[eIdx] = (abs(principalStress1[eIdx] + principalStress3[eIdx]) / 2.0) - Peff;//khodam after Greco paper
//            sigmamI[eIdx] = abs(sigmam[eIdx]) - effectivePressure[eIdx];//khodam
...
//                 safetyFactor[eIdx] = cos(theta) / (taum[eIdx]) * (S0 + (sigmam[eIdx]) * tan(theta));//khodam

                 if (safetyFactor[eIdx]<1)
                      safetyFactor[eIdx]= 1;
                 else if (safetyFactor[eIdx]>5)
                     safetyFactor[eIdx]= 5;
             }
//             std::cout<< "LFS="<< safetyFactor[eIdx]  <<std::endl;
        }
   }
}
        ...
        if (dim >= 2)
            writer.attachCellData(principalStress2, "principal stress 3");
//         if (dim >= 3)
//             writer.attachCellData(principalStress3, "principal stress 3");

        writer.attachCellData(totalEffStressX, "total effective stresses X", dim);
        if (dim >= 2)
            writer.attachCellData(totalEffStressY, "total effective stresses Y", dim);
        if (dim >= 3)
            writer.attachCellData(totalEffStressZ, "total effective stresses Z", dim);
...
//         writer.attachCellData(theta, "Friction Angle");//khodam
//         writer.attachCellData(S0, "Cohesion");//khodam
       // writer.attachCellData(safetyFactor, "Factor of Safety");//khodam
        writer.attachCellData(E, "E");//khodam
        writer.attachCellData(B, "B");//khodam
        writer.attachCellData(nu, "nu");//khodam
...
private:
    ...
    Scalar pnRef_ = 1e5; //to adopt Comsol otherwise it is 1e5;
