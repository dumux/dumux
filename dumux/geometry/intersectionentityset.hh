...
private:
    void build_(const DomainTree& domainTree, const TargetGeometry& targetGeometry)
    {
        ...
        build_(domainTree.entitySet(), *targetEntitySet_, rawIntersections);
    ...
    void build_(const DomainEntitySet& domainEntitySet, const TargetEntitySet& targetEntitySet, const RawIntersections& rawIntersections)
    {
        ...
        for (const auto& rawIntersection : rawIntersections)
        {
            ...
            if constexpr (isMixedDimensional)
            {
                const auto lowDimNeighborIdx = getLowDimNeighborIndex_(rawIntersection);
                for (int i = 0; i < intersectionMap[lowDimNeighborIdx].size(); ++i)
                {
                    if (rawIntersection.cornersMatch(intersectionMap[lowDimNeighborIdx][i]))
                    {
                        ...
                        intersections_[idx].addNeighbors(domainNeighborIndex_(rawIntersection), targetNeighborIndex_(rawIntersection));
                        break;
                    }
                }
            }
            ...
            if(add)
            {
                ...
                if constexpr (isMixedDimensional)
                {
                    intersectionMap[getLowDimNeighborIndex_(rawIntersection)].push_back(rawIntersection.corners());
                    intersectionIndex[getLowDimNeighborIndex_(rawIntersection)].push_back(intersections_.size());
                }
                ...
                intersections_.back().addNeighbors(domainNeighborIndex_(rawIntersection), targetNeighborIndex_(rawIntersection));
            }
        }
    ...
    auto getLowDimNeighborIndex_(const RawIntersection& is)
    {
        if constexpr (dimTarget < dimDomain)
            return targetNeighborIndex_(is);
        else
            return domainNeighborIndex_(is);
    }

    template<class RawIntersection>
    auto domainNeighborIndex_(const RawIntersection& is)
    { return flipNeighborIndices_ ? is.second() : is.first(); }

    template<class RawIntersection>
    auto targetNeighborIndex_(const RawIntersection& is)
    { return flipNeighborIndices_ ? is.first() : is.second(); }
    ...
    bool flipNeighborIndices_ = false;
};
