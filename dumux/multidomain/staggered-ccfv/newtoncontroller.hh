 ...
 */
#ifndef DUMUX_MULTIDOMAIN_NEWTON_CONTROLLER_FOR_STAGGERED_HH
#define DUMUX_MULTIDOMAIN_NEWTON_CONTROLLER_FOR_STAGGERED_HH
...
class MultiDomainNewtonControllerForStaggered;
...
public:
     ...
     * \brief Constructor
     */
    MultiDomainNewtonControllerForStaggered(const Problem &problem)
                           ...
                           const SolutionVector &deltaU)
    {
        ...
        for (std::size_t i = 0; i < uLastIter[stokesIdx][cellCenterIdx].size(); ++i)
        {
            auto uNewI = uLastIter[stokesIdx][cellCenterIdx][i];
            uNewI -= deltaU[stokesIdx][cellCenterIdx][i];

            const Scalar shiftAtDof = model_().relativeShiftAtDof(uLastIter[stokesIdx][cellCenterIdx][i],
                                                                  uNewI);
            shift_ = std::max(shift_, shiftAtDof);
        }

        for (std::size_t i = 0; i < uLastIter[stokesIdx][faceIdx].size(); ++i)
        {
            auto uNewI = uLastIter[stokesIdx][faceIdx][i];
            uNewI -= deltaU[stokesIdx][faceIdx][i];

            const Scalar shiftAtDof = model_().relativeShiftAtDof(uLastIter[stokesIdx][faceIdx][i],
                                                                  uNewI);
            shift_ = std::max(shift_, shiftAtDof);
        }
                      ...
                      SolutionVector &b)
    {
        try
        {
            ...
            const auto& stokesToDarcyBlock = A[stokesIdx][darcyIdx];
            const auto& darcyToStokesBlock = A[darcyIdx][stokesIdx];

            const auto& A11 = stokesBlock[cellCenterIdx][cellCenterIdx];
            const auto& A12 = stokesBlock[cellCenterIdx][faceIdx];
            const auto& A13 = stokesToDarcyBlock[cellCenterIdx][localDarcyIdx];

            const auto& A21 = stokesBlock[faceIdx][cellCenterIdx];
            const auto& A22 = stokesBlock[faceIdx][faceIdx];
            const auto& A23 = stokesToDarcyBlock[faceIdx][localDarcyIdx];

            const auto& A31 = darcyToStokesBlock[localDarcyIdx][cellCenterIdx];
            ...
            assert((A31.N() == A32.N()) && (A32.N() == A33.N()));
...
//                printmatrix(std::cout,M, "M", "");
                      ...
                      const SolutionVector &deltaU)
    {
        ...
        else
        {
            for (std::size_t i = 0; i < uLastIter[stokesIdx][cellCenterIdx].size(); ++i)
            {
                uCurrentIter[stokesIdx][cellCenterIdx][i] = uLastIter[stokesIdx][cellCenterIdx][i];
                uCurrentIter[stokesIdx][cellCenterIdx][i] -= deltaU[stokesIdx][cellCenterIdx][i];
            }
            for (std::size_t i = 0; i < uLastIter[stokesIdx][faceIdx].size(); ++i)
            {
                uCurrentIter[stokesIdx][faceIdx][i] = uLastIter[stokesIdx][faceIdx][i];
                uCurrentIter[stokesIdx][faceIdx][i] -= deltaU[stokesIdx][faceIdx][i];
            }
            for (std::size_t i = 0; i < uLastIter[darcyIdx].size(); ++i)
            {
