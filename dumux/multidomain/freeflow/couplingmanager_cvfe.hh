...
public:
                                ...
                                const SubControlVolumeFace<freeFlowMassIndex>& scvf) const
    {
        ...
        static const bool bubbleMass = getParam<bool>("FreeFlow.UseBubbleInMassBalance", true);
        ...
        const auto ipLocal = element.geometry().local(scvf.ipGlobal());
        if(bubbleMass)
            BubbleLocalBasis::evaluateFunctionWithBubble(element.geometry(), localBasis, ipLocal, shapeValues);
        else
        {
            localBasis.evaluateFunction(ipLocal, shapeValues);
            shapeValues.push_back(0.0);
        }
