...
public:
    ...
    // the constructor
    explicit SubDomainFaceCenteredDiamondLocalAssemblerBase(const Assembler& assembler,
        ...
        for (const auto& scv : scvs(fvGeometry))
        {
            ...
            for (const auto globalJ : stencil)
            {
                ...
                for (int pvIdx = 0; pvIdx < JacobianBlock::block_type::cols; ++pvIdx)
                {
                    ...
                    {
                        const auto bcTypes = this->elemBcTypes()[fvGeometry.boundaryScvf(scv).localIndex()];
