 ...
 */
#ifndef DUMUX_START_BOUNDARY_MULTIDOMAIN_HH
#define DUMUX_START_BOUNDARY_MULTIDOMAIN_HH
           ...
           void (*usage)(const char *, const std::string &))
{
    ...
    using StokesGridCreator = typename GET_PROP_TYPE(StokesProblemTypeTag, GridCreator);
    using DarcyGridCreator = typename GET_PROP_TYPE(DarcyProblemTypeTag, GridCreator);
    ...
    try { StokesGridCreator::makeGrid(); }
    catch (...) {
        std::string usageMessage = "\n\t -> Creation of the stokes grid failed! <- \n\n";
        usageMessage += defaultUsageMessage(argv[0]);
        usage(argv[0], usageMessage);
        throw;
    }
    StokesGridCreator::loadBalance();

    try { DarcyGridCreator::makeGrid(); }
    catch (...) {
        std::string usageMessage = "\n\t -> Creation of the darcy grid failed! <- \n\n";
        usageMessage += defaultUsageMessage(argv[0]);
        usage(argv[0], usageMessage);
        throw;
    }
    DarcyGridCreator::loadBalance();
    ...
    Problem problem(timeManager, StokesGridCreator::grid().leafGridView(), DarcyGridCreator::grid().leafGridView());
