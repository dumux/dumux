            ...
            for (int pvIdx = 0; pvIdx < JacobianBlock::block_type::cols; ++pvIdx)
            {
                ...
                for (const auto& scv : scvs(fvGeometry))
                {
                    for (int eqIdx = 0; eqIdx < numEq; eqIdx++)
                    {
                        ...
                        // For the old boundary interface we set Dirichlet conditions for each scv
                        // For the new boundary interface we enforce constraints globally, i.e. do nothing here
