...
public:
              ...
              typename ParentType::SolutionVectorTuple& curSol)
    {
        ...
        std::cout << problem(freeFlowMomentumIndex).name() << std::endl;
        std::cout << problem(porousMediumIndex).name() << std::endl;

        couplingMapper_.update(*this);

        std::cout << "Sol 0 " << this->curSol(Dune::index_constant<0>())[123] << std::endl;
        std::cout << "Sol 1 " << this->curSol(Dune::index_constant<1>())[123] << std::endl;
    }
