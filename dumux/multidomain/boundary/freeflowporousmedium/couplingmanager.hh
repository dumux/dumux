...
#ifndef DUMUX_FREEFLOW_POROUSMEDIUM_COUPLINGMANAGER_HH
#define DUMUX_FREEFLOW_POROUSMEDIUM_COUPLINGMANAGER_HH
...
#include <dumux/multidomain/genericcouplingmanager.hh>
...
struct CouplingManagers
{
    ...
    using FreeFlowMomentumPorousMediumTraits = MultiDomainTraits<SubDomainTypeTag<freeFlowMomentumIndex>,
                                                                 SubDomainTypeTag<porousMediumIndex>>;

    using FreeFlowMassPorousMediumTraits = MultiDomainTraits<SubDomainTypeTag<freeFlowMassIndex>,
                                                             SubDomainTypeTag<porousMediumIndex>>;
...
: public GenericCouplingManager<MDTraits,
...
{
    using ParentType = GenericCouplingManager<MDTraits,
                                              FreeFlowPorousMediumDetail::CouplingManagerMaps,
                                              typename FreeFlowPorousMediumDetail::CouplingManagers<MDTraits>::FreeFlowCouplingManager,
                                              typename FreeFlowPorousMediumDetail::CouplingManagers<MDTraits>::FreeFlowMomentumPorousMediumCouplingManager,
                                              typename FreeFlowPorousMediumDetail::CouplingManagers<MDTraits>::FreeFlowMassPorousMediumCouplingManager>;
...
public:
              ...
              const SolutionVector& curSol)
    {
        ...
        typename ParentType::template SubCouplingManager<freeFlowMomentumIndex, freeFlowMassIndex>::SolutionVectorTuple ffSolVecTuple;
        ...
        this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).init(freeFlowMomentumProblem, freeFlowMassProblem,
                                                                                std::make_tuple(std::get<freeFlowMomentumIndex>(gridVarsTuple), std::get<freeFlowMassIndex>(gridVarsTuple)),
                                                                                ffSolVecTuple);

        typename ParentType::template SubCouplingManager<freeFlowMassIndex, porousMediumIndex>::SolutionVectorTuple ffMassPmSolVecTuple;
        ...
        this->subCouplingManager(freeFlowMassIndex, porousMediumIndex).init(freeFlowMassProblem, porousMediumProblem, ffMassPmSolVecTuple);

        typename ParentType::template SubCouplingManager<freeFlowMomentumIndex, porousMediumIndex>::SolutionVectorTuple ffMomentumPmSolVecTuple;
        ...
        this->subCouplingManager(freeFlowMomentumIndex, porousMediumIndex).init(freeFlowMomentumProblem, porousMediumProblem, ffMomentumPmSolVecTuple);
    }
                               ...
                               const ElementVolumeVariables<i>& elemVolVars) const
    {
        ...
        auto& couplingContext = this->subCouplingManager(domainI, domainJ).couplingContext(ParentType::globalToLocalDomainIndices(domainI, domainJ).domainI, fvGeometry, scvf);
                                                                 ...
                                                                 const ElementVolumeVariables<freeFlowMomentumIndex>& elemVolVars) const
    {
        ...
        const auto& context = this->subCouplingManager(freeFlowMomentumIndex, porousMediumIndex).couplingContext(domainI, fvGeometry, scvf);
                           ...
                           const SubControlVolumeFace<freeFlowMomentumIndex>& scvf) const
    {
        if (scvf.isFrontal())
        {
            const auto& context = this->subCouplingManager(freeFlowMomentumIndex, porousMediumIndex).couplingContext(Dune::index_constant<freeFlowMomentumIndex>(), fvGeometry, scvf);
        ...
        else
        {
            ...
            const auto& context = this->subCouplingManager(freeFlowMomentumIndex, porousMediumIndex).couplingContext(Dune::index_constant<freeFlowMomentumIndex>(), fvGeometry, frontalScvfOnBoundary);
    ...
    Scalar pressure(const Element<freeFlowMomentumIndex>& element,
                    ...
                    const SubControlVolumeFace<freeFlowMomentumIndex>& scvf) const
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).pressure(element, fvGeometry, scvf);
    }
    ...
    Scalar cellPressure(const Element<freeFlowMomentumIndex>& element,
                        const SubControlVolumeFace<freeFlowMomentumIndex>& scvf) const
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).cellPressure(element, scvf);
    }
    ...
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).density(element, fvGeometry, scvf, considerPreviousTimeStep);
    }

    auto getInsideAndOutsideDensity(const Element<freeFlowMomentumIndex>& element,
                                    const FVElementGeometry<freeFlowMomentumIndex>& fvGeometry,
                                    const SubControlVolumeFace<freeFlowMomentumIndex>& scvf,
                                    const bool considerPreviousTimeStep = false) const
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).getInsideAndOutsideDensity(element, fvGeometry, scvf, considerPreviousTimeStep);
    }
    ...
    Scalar density(const Element<freeFlowMomentumIndex>& element,
                   ...
                   const bool considerPreviousTimeStep = false) const
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).density(element, scv, considerPreviousTimeStep);
    }
    ...
    Scalar effectiveViscosity(const Element<freeFlowMomentumIndex>& element,
                              ...
                              const SubControlVolumeFace<freeFlowMomentumIndex>& scvf) const
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).effectiveViscosity(element, fvGeometry, scvf);
    }
    ...
    {
        return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).faceVelocity(element, scvf);
    }
    ...
    const auto& problem(Dune::index_constant<i> domainI) const
    {
        if constexpr (domainI == freeFlowMomentumIndex)
            return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).problem(ParentType::globalToLocalDomainIndices(freeFlowMomentumIndex, freeFlowMassIndex).domainI);
        else if constexpr (domainI == freeFlowMassIndex)
            return this->subCouplingManager(freeFlowMomentumIndex, freeFlowMassIndex).problem(ParentType::globalToLocalDomainIndices(freeFlowMomentumIndex, freeFlowMassIndex).domainJ);
        else if constexpr (domainI == porousMediumIndex)
            return this->subCouplingManager(porousMediumIndex, freeFlowMassIndex).problem(ParentType::globalToLocalDomainIndices(porousMediumIndex, freeFlowMassIndex).domainI);
        else
    ...
    { return this->subCouplingManager(domainI, domainJ).isCoupled(ParentType::globalToLocalDomainIndices(domainI, domainJ).domainI, scvf); }
    ...
    { return this->subCouplingManager(domainI, domainJ).isCoupled(ParentType::globalToLocalDomainIndices(domainI, domainJ).domainI, scv);}
    ...
    bool isCoupledLateralScvf(Dune::index_constant<freeFlowMomentumIndex> domainI,
                              Dune::index_constant<porousMediumIndex> domainJ,
                              const SubControlVolumeFace<freeFlowMomentumIndex>& scvf) const
    { return  this->subCouplingManager(domainI, domainJ).isCoupledLateralScvf(ParentType::globalToLocalDomainIndices(domainI, domainJ).domainI, scvf); }
                                ...
                                Dune::index_constant<j> domainJ) const
    {
        ...
        return this->subCouplingManager(domainI, domainJ).couplingStencil(ParentType::globalToLocalDomainIndices(domainI, domainJ).domainI,
                                                                          elementI, scvI,
