...
#include <dumux/multidomain/boundary/couplingmapper_intersections.hh>
...
private:
    ...
    using CouplingMapper = MapperCoupledMatchingIntersections<MDTraits, CouplingManagerBinaryCvfe<MDTraits>>;
...
public:
    ...
    decltype(auto) evalCouplingResidual(Dune::index_constant<domainIdx<0>()> domainI,
                                        const LocalAssemblerI& localAssemblerI,
                                        const SubControlVolume<domainIdx<0>()>& scvI,
                                        Dune::index_constant<j> domainJ,
...
private:
                                   ...
                                   const FaceIpData& faceIpData) const
    {
        if constexpr (requires { faceIpData.scvfIndex(); })
        {
            ...
            return faceIpData.intersectionIndex();
        else
