...
private:
    ...
    static_assert(DiscretizationMethods::isCVFE<std::decay_t<decltype(GridDiscretization<0>::discMethod)>>,
        "CouplingManagerBinaryCvfe requires subdomain 0 to be discretized with a CVFE scheme");
    static_assert(DiscretizationMethods::isCVFE<std::decay_t<decltype(GridDiscretization<1>::discMethod)>>,
    ...
    {
        ElementDiscretization<coupledDomainIdx> elemDisc;
...
public:
    ...
    { return makeCouplingContext_(domainI, elemDisc, faceIpData); }
    ...
    {
        const auto eIdx = Dumux::gridDiscretization(this->problem(domainI)).elementMapper().index(element);
    ...
    bool isCoupled(Dune::index_constant<i> domainI,
                   const ElementDiscretization<i>& elemDisc,
                   const typename ElementDiscretization<i>::SubControlVolumeFace& scvf) const
    {
        return isCoupled(domainI, elemDisc, elemDisc.intersectionIndex(scvf));
    }
    ...
    template<std::size_t i>
    bool isCoupled(Dune::index_constant<i> domainI,
                   const typename ElementDiscretization<i>::SubControlVolume& scv) const
    ...
    {
        const auto eIdx = Dumux::gridDiscretization(this->problem(domainI)).elementMapper().index(elemDisc.element());
...
private:
    template<class ElementDiscretization, class FaceIpData>
    std::size_t intersectionIndex_(const ElementDiscretization& elemDisc,
                                   const FaceIpData& faceIpData) const
    {
        if constexpr (requires { faceIpData.scvfIndex(); })
        {
            const auto& scvf = elemDisc.scvf(faceIpData.scvfIndex());
            return elemDisc.intersectionIndex(scvf);
        }
        else if constexpr (requires { faceIpData.boundaryFaceIndex(); })
            return elemDisc.boundaryFace(faceIpData.boundaryFaceIndex()).intersectionIndex();
        else
            DUNE_THROW(Dune::InvalidStateException, "FaceIpData must provide either scvfIndex() or boundaryFaceIndex()");
    }
    ...
    {
        const auto eIdx = Dumux::gridDiscretization(this->problem(domainI)).elementMapper().index(elemDisc.element());
        const auto iIdx = intersectionIndex_(elemDisc, faceIpData);

        if (!couplingMapper_.isCoupled(domainI, eIdx, iIdx))
            DUNE_THROW(Dune::InvalidStateException, "No coupling context found at element " << eIdx << " with intersection index " << iIdx);
        ...
        const auto& otherGridDiscretization = Dumux::gridDiscretization(this->problem(domainJ));
        const auto& otherElement = otherGridDiscretization.element(otherElementIdx);
        auto otherElemDisc = localView(otherGridDiscretization).bind(otherElement);
        const auto elemSol = elementSolution(otherElement, this->curSol(domainJ), otherGridDiscretization);
        return CouplingContext<decltype(elemSol), i, 1-i>{ std::move(otherElemDisc), this->problem(domainJ), std::move(elemSol) };
    }
