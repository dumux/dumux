...
class StokesDarcyCouplingDataImplementationBase
{
    ...
    template<std::size_t id> using FluxVariables = typename GET_PROP_TYPE(SubDomainTypeTag<id>, FluxVariables);
...
public:
                                     ...
                                     const SubControlVolumeFace<stokesIdx>& scvf) const
    {
        ...
        //fluxVars.init(couplingManager_.problem(darcyIdx), stokesContext.element, stokesContext.fvGeometry, elemVolVars, darcyScvf, elemFluxVarsCache);
        ...
        {
            // v = -K/mu * (gradP + rho*g)
            const auto& volVarsDarcy = stokesContext.volVars;
            const Scalar velocity = stokesElemFaceVars[scvf].velocitySelf();
            const Scalar mu = volVarsDarcy.viscosity(darcyPhaseIdx);
            const Scalar rho = volVarsDarcy.density(darcyPhaseIdx);
            //Scalar tij = elemFluxVarsCache[darcyScvf].advectionTij();
            ...
            if (fluxVarsCache.usesSecondaryIv())
            {
                ...
                const auto& pj = fluxVarsCache.pressuresSecondaryIv(darcyPhaseIdx);
                const auto ivIndexInContainer = fluxVarsCache.ivIndexInContainer();
                const auto& interactionVolume = elemFluxVarsCache.secondaryInteractionVolumes()[ivIndexInContainer];
                ...
                {
                    const auto& curLocalScvf = interactionVolume.localScvf(faceIdx);
                    const auto& curGlobalScvf = stokesContext.fvGeometry.scvf(curLocalScvf.globalScvfIndex());
