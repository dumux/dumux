 ...
 * \copydoc Dumux::MapperCoupledMatchingBoundaryFaces
 */
...
{
    template<std::size_t i> using GridDiscretization = typename MDTraits::template SubDomain<i>::GridGeometry;
    template<std::size_t i> using GridView = typename GridDiscretization<i>::GridView;
...
public:
    ...
    void update(const CouplingManager& couplingManager)
    {
        ...
        const auto& gridDisc0 = Dumux::gridDiscretization(problem0);
        const auto& gridDisc1 = Dumux::gridDiscretization(problem1);

        isCoupledDof_[0].assign(gridDisc0.numDofs(), false);
        isCoupledDof_[1].assign(gridDisc1.numDofs(), false);

        auto elemDisc0 = localView(gridDisc0);
        auto elemDisc1 = localView(gridDisc1);

        for (const auto& element1 : elements(gridDisc1.gridView()))
        {
            elemDisc1.bindElement(element1);

            // the outside elements already added to the stencils for this element,
            // which may touch the coupling interface with more than one intersection
            std::vector<std::size_t> coupledOutsideElements;

            for (const auto& boundaryFace1 : boundaryFaces(elemDisc1))
            {
                ...
                const auto indices = intersectingEntities(globalPos, gridDisc0.boundingBoxTree());
                ...
                const auto eIdx1 = gridDisc1.elementMapper().index(element1);
                const auto eIdx0 = indices[0];
                const auto& element0 = gridDisc0.element(eIdx0);
                elemDisc0.bindElement(element0);

                // add the element dofs once per pair of coupled elements
                if (std::find(coupledOutsideElements.begin(), coupledOutsideElements.end(), eIdx0) == coupledOutsideElements.end())
                {
                    coupledOutsideElements.push_back(eIdx0);

                    for(auto&& localDof : localDofs(elemDisc1))
                        stencils_[0][eIdx0].push_back(localDof.dofIndex());

                    for(auto&& localDof : localDofs(elemDisc0))
                        stencils_[1][eIdx1].push_back(localDof.dofIndex());
                }

                for (const auto& boundaryFace0 : boundaryFaces(elemDisc0))
                {
                    ...
                    for(const auto& localDof : localDofs(elemDisc0, boundaryFace0))
                        isCoupledDof_[0][localDof.dofIndex()] = true;

                    for(const auto& localDof : localDofs(elemDisc1, boundaryFace1))
        ...
        // dofs are shared between elements, so the same dof may have been added several times.
        // Duplicate entries would make the coupling derivatives be accumulated more than once.
        for (auto& stencil : stencils_)
            for (auto& [eIdx, dofs] : stencil)
            {
