...
#include <dune-multidomaingrid/dune/grid/multidomaingrid/multidomaingrid.hh>
...
public:
      ...
      newtonCtl_(asImp_())
    {
        ...
        asImp_.initializeGrid(); // initialize multidomaingrid
...
//         if (!useIterativeSolver_)
    ...
    /*!
     * \brief Initialization of the multidomaingrid and the subdomaingrids
     */
    void initializeGrid()
    {
        ...
        for (auto eIt = this->mdGrid().template leafbegin<0>();
             eIt != this->mdGrid().template leafend<0>(); ++eIt)
             {
                 ...
                 if (eIt->partitionType() == Dune::InteriorEntity)
                 {
                     if (globalPos[dim - 1] > interfaceVerticalPos)
                     {
                         ...
                         // TODO Darcy domain might be shorter than Stokes domain
                     {
             ...
             darcyElementIndices_.resize(this->sdGridViewDarcy().size(0)); // TODO warum nur Darcy?
             ...
             for (auto eIt = this->sdGridDarcy().template leafbegin<0>();
                  ...
                  } // TODO warum nicht Stokes?
    }

    //! Returns the vertical position of the interface
...
//         }
    ...
    //! Returns a const reference to the grid of the Stokes subdomain
...
//     { return darcyProblem_; }
    ...
    //! Returns a reference to the multidomain grid
    MultiDomainGrid& mdGrid() //
    { return *mdGrid_; }

    //! Returns a const reference to the multidomain grid
    const MultiDomainGrid& mdGrid() const //
    { return *mdGrid_; }

     //! Returns the multidomain gridview
     ...
     */
    Model &model()
    { return model_; } // TODO return sdProblemDarcy().model()?
    ...
    { return newtonCtl_; } // TODO return sdProblemDarcy().newtonController() ??
...
//     bool verbose_;
    ...
    // TODO: funktionen mit '//' == erledigt
    // funktionen mit  '// <-- ?' == hier vorhanden, nicht in multidomain von T&C
    // in multidomain von T&C:
    // - reference to localresidual? LocalResidualStoeks, LocalResidualDarcy
