...
public:
    ...
    LocalContext couplingContext(Dune::index_constant<i> domainI,
                                 const Element<i>& elementI,
 ...
 * \brief The interface of the coupling manager for multi domain problems
 */
template<class Traits, class LocalContext = EmptyCouplingContext>
class CouplingManager
{
    template<std::size_t id> using SubDomainTypeTag = typename Traits::template SubDomain<id>::TypeTag;
    template<std::size_t id> using PrimaryVariables = GetPropType<SubDomainTypeTag<id>, Properties::PrimaryVariables>;
    template<std::size_t id> using GridView = typename Traits::template SubDomain<id>::GridGeometry::GridView;
    template<std::size_t id> using Element = typename GridView<id>::template Codim<0>::Entity;
    template<std::size_t id> using Problem = typename Traits::template SubDomain<id>::Problem;
    template<std::size_t id> using ProblemPtr = const Problem<id> *;
    using ProblemPtrs = typename Traits::template Tuple<ProblemPtr>;
...
} // end namespace Dumux::MultiDomain
