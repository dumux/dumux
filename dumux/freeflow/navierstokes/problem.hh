 ...
 */
template<class TypeTag>
class NavierStokesProblemImpl<TypeTag, DiscretizationMethods::Cvfe>
: public FVProblemWithSpatialParams<TypeTag>
{
    using ParentType = FVProblemWithSpatialParams<TypeTag>;
    using Implementation = GetPropType<TypeTag, Properties::Problem>;
...
public:
    ...
    void applyInitialSolution(SolutionVector& sol) const
    {
        static_assert(GridGeometry::discMethod == DiscretizationMethods::cvfe);
        sol.resize(this->gridGeometry().numDofs());
        std::vector<bool> dofHandled(this->gridGeometry().numDofs(), false);
        auto fvGeometry = localView(this->gridGeometry());
        for (const auto& element : elements(this->gridGeometry().gridView()))
...
template<class TypeTag>
class NavierStokesProblemImpl<TypeTag, DiscretizationMethods::Box> : public NavierStokesProblemImpl<TypeTag, DiscretizationMethods::CCTpfa>
{
    using ParentType = NavierStokesProblemImpl<TypeTag, DiscretizationMethods::CCTpfa>;
    using GridGeometry = GetPropType<TypeTag, Properties::GridGeometry>;
    using CouplingManager = GetPropType<TypeTag, Properties::CouplingManager>;
