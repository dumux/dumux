...
#ifndef DUMUX_NAVIER_STOKES_PHASEFIELD_MODEL_HH
#define DUMUX_NAVIER_STOKES_PHASEFIELD_MODEL_HH
...
struct NavierStokesPhasefieldModelTraits : public IsothermalT
{
    //! Export the isothermal model traits
    using IsothermalTraits = IsothermalT;

    //! We solve for one more equation, i.e. the phasefield equation
    static constexpr int numEq() { return IsothermalTraits::numEq()+1; }

    //! We additionally solve for the phasefield equation
    static constexpr bool enablePhasefield() { return true; }

    //! the indices
    using Indices = NavierStokesPhasefieldIndices<typename IsothermalTraits::Indices, numEq()>;
};
