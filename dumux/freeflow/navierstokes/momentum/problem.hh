...
public:
    ...
    {
        return asImp_().constraintInfoAtPos(fvGeometry.dofPosition(localDof));
    }

    /*!
     * \brief Set at constraint at a given position
     *
     * \param globalPos The position of a local dof
     */
    ConstraintInfo constraintInfoAtPos(const GlobalPosition& globalPos) const
    {
        // As default we don't have any constraints
        return ConstraintInfo{};
    }

    /*!
     * \brief Specifies the values related to constraint information
     *
     * \param fvGeometry The finite-volume geometry
     * \param localDof The local dof
     * \param constraintInfo The constraint information
     */
    template<class LocalDof>
    ...
    {
        return asImp_().constraintAtPos(fvGeometry.dofPosition(localDof));
    }
    ...
    {
        DUNE_THROW(Dune::NotImplemented, "A constraint is set but not constraint values are defined in the problem.");
    }
    ...
    /*!
     * \brief Evaluate the initial value at a localDof
     */
    template<class LocalDof>
    InitialValues initial(const FVElementGeometry& fvGeometry, const LocalDof& localDof) const
    {
        return asImp_().initialAtPos(fvGeometry.dofPosition(localDof));
    }
