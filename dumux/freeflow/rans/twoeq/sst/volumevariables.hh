...
public:
                              ...
                              const SubControlVolume& scv)
    {
        ...
        if (problem.useStoredEddyViscosity())
            ...
            RANSParentType::setDynamicEddyViscosity_(calculateEddyViscositySST());
        }
     ...
     */
    Scalar calculateEddyViscositySST()
    {
        using std::sqrt;
        using std::max;
    ...
    /*!
     * \brief Returns the dynamic eddy viscosity \f$\mathrm{[Pa s]}\f$ for the BSL-model.
     */
    Scalar calculateEddyViscosityBSL()
    ...
    { return std::sqrt(2*vorticityTensorScalarProduct_); }
    ...
    Scalar F1() const
    {
        ...
        for (unsigned int i = 0; i < Traits::ModelTraits::dim(); ++i){
            gradientProduct += storedTurbulentKineticEnergyGradient_[i] * storedDissipationGradient_[i];}

        Scalar positiveCrossDiffusion = 2.0 * RANSParentType::density() * sigmaOmega2() / storedDissipation_ * gradientProduct;
        positiveCrossDiffusion = std::max(positiveCrossDiffusion,1e-20);

        const Scalar possibleMin2 = (4.0 * RANSParentType::density() * sigmaOmega2() * storedTurbulentKineticEnergy_)
                                  / (positiveCrossDiffusion * wallDistance_ * wallDistance_);

        const Scalar possibleMax1 = (std::sqrt(storedTurbulentKineticEnergy_)) / (0.09 * storedDissipation_ * wallDistance_);
        const Scalar possibleMax2 = (500.00 * kinematicViscosity_) / (wallDistance_ * wallDistance_ * storedDissipation_);
    ...
    Scalar F2() const
    {
        const Scalar possibleMax1 = 2.0 * (std::sqrt(storedTurbulentKineticEnergy_)) / (0.09 * storedDissipation_ * wallDistance_);
        const Scalar possibleMax2 = (500.0 * kinematicViscosity_) / (wallDistance_ * wallDistance_ * storedDissipation_);
