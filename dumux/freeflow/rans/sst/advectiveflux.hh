 ...
 * \brief Helper struct defining the advective fluxes of the two-equation SST turbulence closure
 *        fused onto the single-phase Navier-Stokes mass model, for use with
 *        dumux/freeflow/navierstokes/scalarfluxhelper.hh's boundary flux helpers (the outflow
 *        treatment at the test's outlet, see test/freeflow/rans/sst/channel).
 */
#ifndef DUMUX_RANS_SST_MASS_ADVECTIVE_FLUX_HH
#define DUMUX_RANS_SST_MASS_ADVECTIVE_FLUX_HH
...
#ifndef DOXYGEN
struct SSTMassModelTraits;
#endif

/*!
 * \ingroup FreeflowModels
 * \brief Helper struct defining the advective fluxes of the SST turbulence closure fused onto
 *        the single-phase Navier-Stokes mass model: the pressure equation's mass flux (density
 *        upwind, as in the plain 1p model) plus k's and omega's advective fluxes (rho*k,
 *        rho*omega upwind).
 */
template<class T>
struct AdvectiveFlux<SSTMassModelTraits, T>
{
