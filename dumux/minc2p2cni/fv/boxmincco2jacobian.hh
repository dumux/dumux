...
public:
	...
	virtual VBlockType computeM(const Entity& e, const VBlockType* sol,
		...
		int globalIdx = this->vertexMapper.template map<dim>(e,
				sfs[node].entity());
		...
		for (int nC = 0; nC < numCont; nC++) {
				result[water][nC]
						= sNDat[globalIdx].porosity[nC]
								*(varData[node].density[wPhase][nC]
										*varData[node].saturation[water][nC]
										*varData[node].massfracMincW[water][nC]
										+varData[node].density[nPhase][nC]
												*varData[node].saturation[co2][nC]
												*varData[node].massfracMincN[water][nC]);
				result[co2][nC] = sNDat[globalIdx].porosity[nC]
						*(varData[node].density[nPhase][nC]
								*varData[node].saturation[co2][nC]
								*varData[node].massfracMincN[co2F][nC]
								+varData[node].density[wPhase][nC]
										*varData[node].saturation[water][nC]
										*varData[node].massfracMincW[co2][nC]);
				result[heat][nC] = sNDat[globalIdx].porosity[nC]
						* (varData[node].density[wPhase][nC]
								* varData[node].intenergy[wPhase][nC]
								* varData[node].saturation[water][nC]
								+ varData[node].density[nPhase][nC]
										* varData[node].intenergy[nPhase][nC]
										* varData[node].saturation[co2][nC])
						+ elData.heatCap * varData[node].temperature[nC];
		}
	...
	virtual VBlockType computeA(const Entity& e, const VBlockType* sol, int face) {
		...
		int globalIdx_i = this->vertexMapper.template map<dim>(e,
				sfs[i].entity());
		int globalIdx_j = this->vertexMapper.template map<dim>(e,
				sfs[j].entity());
	...
	virtual VBlockType computeQ(const Entity& e, const VBlockType* sol,
			const int& node) {
		// ASSUME problem.q already contains \rho.q
		VBlockType q(0.0);
		q = problem.q(this->fvGeom.subContVol[node].global, e,
				this->fvGeom.subContVol[node].local);
		...
		KF = this->problem.soil().KFracture(globalIdx_i, e, local_i);
		KM = this->problem.soil().KMatrix(globalIdx_i, e, local_i);
...
//			}
	 ...
	 */
	virtual void primaryVarSwitch(const Entity& e, int global, VBlockType* sol,
			int local) {
...
//		  	  {
		...
		for (int nC=0; nC<numCont; nC++){
			...
			pW[nC] = sol[local][3*nC];
			if (state == bothPhases){satW[nC]=1.0 - sol[local][3*nC+1];}
			if (state == waterPhase){satW[nC]=1.0;}
			if (state == gasPhase){satW[nC]=0.0;}

			//TODO introduce the pN
			pC[nC] =  problem.materialLaw().pC(satW[nC], this->fvGeom.subContVol[global].global, e,
					this->fvGeom.subContVol[global].local);
			pN[nC] = pW[nC]+pC[nC];
			switch (state) {
			case gasPhase:
				Scalar xWNmass;
				xWNmass = sol[local][3*nC+1];
				...
				if (xWNmass > 0.001 && switched == false) {
					...
					sol[local][3*nC+1] = 1.0 - 1.e-6; // initialize solution vector
					switched = true;
				}
				break;
			...
			case waterPhase:
				...
				xAWmass = sol[local][3*nC+1];
				xAWmax = problem.multicomp().xAW(pN[nC], sol[local][3*nC+2]);
				...
				if (xAWmass > xAWmax && switched == false) {
					...
					std::cout << "Gas appears at node " << global
							<< "  Coordinates: " << Coordinates << std::endl;
					sNDat[global].phaseState[nC] = bothPhases;
					sol[local][3*nC+1] = 1.e-6; // initialize solution vector
					switched = true;
				}
				break;

			case bothPhases:
				Scalar satNFracture = sol[local][3*nC+1];
				if (satNFracture < 0.0 && switched == false) {
					...
					sol[local][3*nC+1] = 1e-6; // initialize solution vector
					switched = true;
				}
...
////MINC
	...
	virtual void updateStaticData(const Entity& e, VBlockType* sol) {
		...
		for (int k = 0; k < sfs.size(); k++) {
			const int globalIdx = this->vertexMapper.template map<dim>(e,
					sfs[k].entity());

			for (int nC=0; nC < numCont; nC++){
				if (nC==0){
					sNDat[globalIdx].porosity[nC] = problem.soil().porosityFracture(this->fvGeom.subContVol[k].global, e,
							this->fvGeom.subContVol[k].local);
				}
				else {
					sNDat[globalIdx].porosity[nC] = problem.soil().porosityMatrix(this->fvGeom.subContVol[k].global, e,
						this->fvGeom.subContVol[k].local);
				}
	...
	virtual void initiateStaticData(const Entity& e) {
		...
		for (int k = 0; k < sfs.size(); k++) {
			...
			if (!sNDat[globalIdx].visited) {
				...
				sNDat[globalIdx].porosityFracture = problem.soil().porosityFracture(this->fvGeom.subContVol[k].global, e,
						this->fvGeom.subContVol[k].local);
				sNDat[globalIdx].porosityMatrix = problem.soil().porosityMatrix(this->fvGeom.subContVol[k].global, e,
										this->fvGeom.subContVol[k].local);
				// mark elements that were already visited
				sNDat[globalIdx].visited = true;
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		for (int nC = 0; nC < numCont; nC++) {
			...
			varData[i].pC[nC] = problem.materialLaw().pC(varData[i].saturation[wPhase][nC],
					this->fvGeom.subContVol[i].global, e, this->fvGeom.subContVol[i].local); //Capillary pressure
...
//			std::cout << "temperature: " << varData[i].temperature[nC]<<"pressure :"<< varData[i].p[nPhase][nC]<<"node "<<i<<"\n";
			varData[i].mobility[wPhase][nC] = problem.materialLaw().mobW(varData[i].saturation[wPhase][nC],
							this->fvGeom.subContVol[i].global, e, this->fvGeom.subContVol[i].local, varData[i].temperature[nC],
							varData[i].p[wPhase][nC]);

			varData[i].lambda[nC] = problem.soil().heatCond(this->fvGeom.subContVol[i].global, e,
			...
			varData[i].krCO2[nC] = problem.materialLaw().krn(varData[i].saturation[nPhase][nC],
					this->fvGeom.subContVol[i].global, e,
