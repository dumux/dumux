...
template<class G, class RT, class VC> class FVTransSubProbs :
    public TransportSubProbs< G, RT, VC> {
...
public:
     ...
     */
    FVTransSubProbs(G& g, FractionalFlowProblemSubProbs<G, RT, VC>& prob, int lev = 0,
         bool rec = false, double amax = 0.8, const NumericalFlux<RT>& numFl = *(new Upwind<RT>)) :
        TransportSubProbs<G, RT, VC>(g, prob, lev),
                elementmapper(g, g.levelIndexSet(lev)),// gridview(g.levelView(lev)),
//                indexset(gridview.indexSet())
                indexset(g.levelIndexSet(lev)),
                reconstruct(rec),numFlux(numFl), alphamax(amax)
                {}
...
template<class G, class RT, class VC> int FVTransSubProbs<G, RT, VC>::update(const RT t, RT& dt,
    ...
    for (Iterator it = this->grid.template lbegin<0>(this->level()); it != eendit; ++it) {
        ...
        for (IntersectionIterator
            ...
            *= Dune::ReferenceElements<ct,dim-1>::general(gtf).volume();
            ...
            double factor, totfactor;//, volumecorrectionfactor;
...
//                    }
                    ...
                    double fI = this->transproblem.materialLaw.fractionalW(satI, global, *Hostit,local);
                    double fJ = this->transproblem.materialLaw.fractionalW(satJ, nbglobal, *Hostoutside, nblocal);

                    factor = 0;

                    factor += (velocityJI*numFlux(satJ, satI, fJ, fI)
...
//                    std::cout<<"totfactorneighbor = "<<totfactor<<std::endl;
//                    std::cout<<"neighbor : "<<fI<<"  "<<fJ<<"  "<<satI<<"  "<<satJ<<std::endl;
                }
            }
...
//                    }
                    ...
                    double fI = this->transproblem.materialLaw.fractionalW(satI, global, *Hostit,local);
                    double fBound = this->transproblem.materialLaw.fractionalW(satBound, faceglobal, *Hostit, facelocalDim);

                    factor = 0;
                    factor += (velocityJI*numFlux(satBound, satI, fBound, fI)
...
//                    std::cout<<"totfactordirichlet = "<<totfactor<<std::endl;
//                    std::cout<<"dirichlet : "<<fI<<"  "<<fBound<<"  "<<satI<<"  "<<satBound<<std::endl;
                }
                else
                {
                    double satI = this->transproblem.variables.saturation[indexi];
...
//                    std::cout<<"helpfactor = "<<helpfactor<<std::endl;
                    factor = this->transproblem.JSat(faceglobal, *it, facelocalDim, helpfactor);
                    totfactor = 0;
//                    std::cout<<"totfactorneumann = "<<totfactor<<std::endl;
                }
            }
        ...
        updateVec[indexi] /=this->transproblem.soil.porosity(global, *Hostit,local);
        RT volumecorrectionfactor = (1-this->transproblem.soil.Sr_w(global, *Hostit,local)-this->transproblem.soil.Sr_n(global, *Hostit,local))*this->transproblem.soil.porosity(global, *Hostit,local);
//        std::cout<<volumecorrectionfactor<<std::endl;
...
template<class G, class RT, class VC> void FVTransSubProbs<G, RT, VC>::initialTransport() {
//    std::cout<<"initsat = "<<&this->transproblem.variables.saturation<<std::endl;
    ...
    for (Iterator it = this->grid.template lbegin<0>(this->level()); it != eendit; ++it) {
        ...
        const Dune::FieldVector<ct,dimworld> global = it->geometry().global(local);
...
//template<class G, class SG, class RT, class VC> void FVTransSubProbs<G, SG, RT, VC>::CalculateSlopes(
...
//        // run through all intersections with neighbors and boundary
//        IntersectionIterator
//                isend = IntersectionIteratorGetter<SG, LevelTag>::end(*it);
//        for (IntersectionIterator
//                is = IntersectionIteratorGetter<SG, LevelTag>::begin(*it); is
//                !=isend; ++is) {
//            // local number of facet
//            int numberInSelf = is->numberInSelf();
//
