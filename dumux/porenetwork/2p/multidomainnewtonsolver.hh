...
template <class Assembler, class LinearSolver, class CouplingManager,
          ...
          template<class, class> class NewtonConsistencyChecks = Dumux::PoreNetwork::TwoPNewtonConsistencyChecks>
class MultiDomainNewtonSolver : public Dumux::MultiDomainNewtonSolver<Assembler, LinearSolver, CouplingManager, Reassembler, Comm>
{
    using ParentType =  Dumux::MultiDomainNewtonSolver<Assembler, LinearSolver, CouplingManager, Reassembler, Comm>;
            ...
            forEach(std::make_index_sequence<Assembler::Traits::numSubDomains>{}, [&](auto&& id)
            {
                ...
                if constexpr (hasInvasionState)
                {
                    ...
                    else
                    {
                        ...
                        for (std::size_t i = 0; i < uLastIterPNM.size(); ++i)
                        {
                            saturationChop_(uCurrentIterPNM[i][Indices::saturationIdx],
                                            uLastIterPNM[i][Indices::saturationIdx]);
