 ...
 */
#ifndef DUMUX_PNM_MPNC_GRID_FLUXVARSCACHE_HH
#define DUMUX_PNM_MPNC_GRID_FLUXVARSCACHE_HH
 ...
 */
template<class P, class FVC, class IS = MPNCInvasionState<P>>
struct PNMMPNCDefaultGridFVCTraits
{
    using Problem = P;
    ...
    using LocalView = PNMMPNCElementFluxVariablesCache<GridFluxVariablesCache, cachingEnabled>;
};
...
template<class Problem,
         ...
         bool cachingEnabled,
         class Traits>
class PNMMPNCGridFluxVariablesCache;
...
class PNMMPNCGridFluxVariablesCache<P, FVC, true, Traits>
{
    using Problem = typename Traits::Problem;
    using ThisType = PNMMPNCGridFluxVariablesCache<P, FVC, true, Traits>;
...
public:
    ...
    PNMMPNCGridFluxVariablesCache(const Problem& problem)
    : problemPtr_(&problem)
                ...
                bool forceUpdate = true)
    {
        ...
        for (const auto& element : elements(gridGeometry.gridView()))
        {
            ...
            for (auto&& scvf : scvfs(fvGeometry))
                cache(eIdx, scvf.index()).update(problem(), element, fvGeometry, elemVolVars, scvf, invasionState().invaded(element), invasionState().transmissibilityFactor());
        }
    }
 ...
 */
template<class P, class FVC, class Traits>
class PNMMPNCGridFluxVariablesCache<P, FVC, false, Traits>
{
    using Problem = typename Traits::Problem;
    using ThisType = PNMMPNCGridFluxVariablesCache<P, FVC, false, Traits>;
    using InvasionState = typename Traits::InvasionState;
