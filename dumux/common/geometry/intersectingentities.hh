...
intersectingEntities(const Dune::FieldVector<ctype, dimworld>& point,
                     const BoundingBoxTree<EntitySet>& tree, bool isCartesian = false)
{
    // Call the recursive find function to find candidates
    std::vector<std::size_t> entities;
    intersectingEntities(point, tree, tree.numBoundingBoxes() - 1, entities, isCartesian);
    return entities;
}
                          ...
                          bool isCartesian)
{
    ...
    {
        intersectingEntities(point, tree, bBox.child0, entities, isCartesian);
        intersectingEntities(point, tree, bBox.child1, entities, isCartesian);
    }
}
