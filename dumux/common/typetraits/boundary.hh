...
//! helper struct detecting if a problem has new boundaryTypes function
template<class P, class FVG, class INT>
using NewBoundaryTypesFunctionDetector = decltype(
    std::declval<P>().boundaryTypes(std::declval<FVG>(), std::declval<INT>())
);

template<class P, class FVG, class INT>
constexpr inline bool hasProblemNewBoundaryTypesFunction()
{ return Dune::Std::is_detected<NewBoundaryTypesFunctionDetector, P, FVG, INT>::value; }

//! helper struct to determine BoundaryTypes related to provided boundaryTypes(...) function of the problem
template<class P, class FVG, class INT, bool newInterface = hasProblemNewBoundaryTypesFunction<P, FVG, INT>()>
