...
int main(int argc, char** argv)
{
  try{
    ...
    double depthBOR = 3238.20;  // bottom of reservoir
...
//    typedef Dune::ALUSimplexGrid<dim,dim> GridType;
    ...
    //grid.createLGMGrid(argv[1]);
     ...
     Dune::Benchmark3Soil<GridType, NumberType> soil(grid, true, "properties_johansen.dat");
     Dune::TwoPhaseRelations<GridType, NumberType> materialLaw(soil, wPhase, nPhase);
	 Dune::CBrineCO2 multicomp(wPhase, nPhase);

     Dune::Benchmark3Problem<GridType, NumberType> problem(wPhase, nPhase, soil, materialLaw, multicomp, depthBOR);
