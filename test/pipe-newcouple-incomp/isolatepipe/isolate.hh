...
public:
    ...
    bool operator() (const HostVertexPointer& p1, const HostVertexPointer& p2) const
    {
        ...
        sprintf(stringP1,
                "%e%e%e",
                p1->geometry().corner(0)[0],
                p1->geometry().corner(0)[1],
                p1->geometry().corner(0)[2]);
        char stringP2[100];
        sprintf(stringP2,
                "%e%e%e",
                p2->geometry().corner(0)[0],
                p2->geometry().corner(0)[1],
                p2->geometry().corner(0)[2]);
...
public:
    ...
    bool boundary();
    double length(const VectorVertexOnLineType& vectorVertexOnLine) const;
...
template <class GridType>
double VertexOnLine<GridType>::length(const VectorVertexOnLineType& vectorVertexOnLine) const
{
    double l=0.0;
    for(unsigned i=0; i < indexVertexVectorOnLine.size(); i++)
    {
        ...
        distVec =
            vectorVertexOnLine[indexVertexVectorOnLine[i]].nodePoint->geometry().corner(0) -
            nodePoint->geometry().corner(0);
