...
struct BenchmarkResult
{
                  ...
                  SolutionType& solution, bool pureNeumann = false)
    {
        ...
        for (Iterator it = gridview.template begin<0>(); it != eendit; ++it)
        {
            ...
            // update sumf
            sumf += volume*problem.sourcePress(global, element, local);

            // get the absolute permeability
            FieldMatrix<double,dim,dim> K = problem.soil().K(global, element, local);
...
struct ResultEvaluation
{
	...
	void evaluate(const GridView& gridView, Problem& problem,
			const SolVector& solution, const VelVector& velocity)
	{
		...
		for (ElementIterator eIt = gridView.template begin<0>(); eIt != endEIt; ++eIt)
		{
			...
			*Dune::ReferenceElements<Scalar,dim>::general(geomType).volume();
			...
			Scalar approxPressure = solution.evallocal (0, element, local);
			...
			FieldVector<Scalar,2*dim> fluxVector;
			FieldVector<Scalar,dim> exactGradient;
			...
			for (IntersectionIterator is = gridView.ibegin(element); is!=endis; ++is)
			{
				...
				// get the approximate normalvelocity
				double approximateFlux = velocity[eIdx][i]*unitOuterNormal;
			...
			// get the approximate gradient
			FieldVector<Scalar,dim> approximateGradient;
			K.solve(approximateGradient, elementVelocity);
			...
			// the difference between exact and approximate gradient
			FieldVector<Scalar,dim> gradDiff(exactGradient);
			gradDiff += approximateGradient;
