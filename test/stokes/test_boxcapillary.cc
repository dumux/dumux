...
#include "capillaryflowproblem.hh"
...
int main(int argc, char** argv)
{
    try{
        ...
        if (argc != 2 && argc != 3) {
            std::cout << "Usage: test_boxcapillary dgffilename [refinementsteps]" << std::endl;
            return (1);
        }
        ...
        Dune::CapillaryFlowProblem<GridType, double> problem;
        typedef Dune::LeafP1BoxStokes<GridType, NumberType, dim> BoxStokes;
        BoxStokes boxStokes(grid, problem);

        Dune::TimeLoop<GridType, BoxStokes> timeloop(0, 1, 1, "test_boxcapillary", 1);
