    ...
    // the members of the struct are defined here
     ...
     void updateVariableData(const Element& element, const SolutionVector* sol, int i, std::vector<VariableNodeData>& varData)
     {
    	 varData[i].pN = sol[i][4];
     	 ...
     	 varData[i].viscosity = 1.65e-5;
     	 varData[i].internalenergy = problem.gasPhase().intEnergy(283.15, varData[i].pN, 0); //problem.internalenergy(this->fvGeom.elementGlobal, element, this->fvGeom.elementLocal);
     	 varData[i].enthalpy = problem.gasPhase().enthalpy(283.15, varData[i].pN, 0); //problem.enthalpy(this->fvGeom.elementGlobal, element, this->fvGeom.elementLocal);
