...
class APS2 : public StokesProblem<Grid, Scalar>
{
...
public:
                                        ...
                                        const FieldVector<Scalar,dim>& localPos) const
    {
        ...
        result[0] = 1/8.0*pi*(16*pi*cos(pi*x[1])*sin(pi*x[0]) +
							  cos(2*pi*x[1])*sin(2*pi*x[0]));

        result[1] = -2*pi*pi*cos(pi*x[0])*sin(pi*x[1]) +
					 1/8.0*pi*cos(2*pi*x[0])*sin(2*pi*x[1]);

        result[2] = 1/2.0*pi*pi*cos(2*pi*x[0])*cos(2*pi*x[1]);
...
//        if (globalPos[0] < 1e-6 || globalPos[1] < 1e-6 || globalPos[1] > 1 - 1e-6)
                                        ...
                                        const FieldVector<Scalar,dim>& localPos) const
    {
        ...
        result[0] =  sin(pi*x[0])*cos(pi*x[1]);
        result[1] = -cos(pi*x[0])*sin(pi*x[1]);
   ...
   virtual Scalar beaversJosephC(const FieldVector<Scalar,dim>& globalPos, const Element& element,
    ...
    virtual FieldMatrix<Scalar, dim, dim> velocityGradient(const FieldVector<Scalar,dim>& x) const
    {
        ...
        result[0][0] =  pi*cos(pi*x[0])*cos(pi*x[1]);
        result[0][1] = -pi*sin(pi*x[0])*sin(pi*x[1]);
        result[1][0] =  pi*sin(pi*x[0])*sin(pi*x[1]);
        result[1][1] = -pi*cos(pi*x[0])*cos(pi*x[1]);
