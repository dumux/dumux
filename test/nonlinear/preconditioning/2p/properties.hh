...
struct TwoPIncompressibleTpfaCached { using InheritsFrom = std::tuple<TwoPIncompressibleTpfa>; };
} // end namespace TTag

// SPE11 and other production setups cache the grid variables, which changes how a subdomain solve must
// refresh them; this tag exists so that path is exercised rather than assumed
template<class TypeTag>
struct EnableGridVolumeVariablesCache<TypeTag, TTag::TwoPIncompressibleTpfaCached> { static constexpr bool value = true; };
template<class TypeTag>
struct EnableGridFluxVariablesCache<TypeTag, TTag::TwoPIncompressibleTpfaCached> { static constexpr bool value = true; };
