...
int main(int argc, char* argv[])
{
    ...
    const std::vector<ImplicitMethodData> implicitMethods = {
    ...
    {
        const auto maxModulus = sampleLeftHalfPlaneMaxModulus(entry.method);
        expect(maxModulus <= 1.0 + 1e-10,
               "Sampled left-half-plane stability violated for " + entry.method->name());
    }
    ...
    for (const auto& entry : implicitMethods)
    {
        auto& node = out["methods"][entry.id];
        node["name"] = displayName(entry.id, entry.method);
        node["stablePoints"] = sampleStablePoints(entry.method, plotLimit, plotResolution);
    }
