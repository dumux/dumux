...
public:
    ...
    void update(Scalar time = 0.0)
    {
        ...
        analyticalConcentration_.resize(problem_->gridGeometry().numCellCenterDofs());
        ...
        for (const auto& element : elements(problem_->gridGeometry().gridView()))
        {
            ...
            for (const auto& scv : scvs(fvGeometry))
            {
                ...
                {
                    analyticalVelocityOnFace_[scvf.dofIndex()][scvf.directionIndex()] = problem_->analyticalSolution(scvf.center())[Indices::velocity(scvf.directionIndex())];
                }

                analyticalPressure_[scv.dofIndex()] = problem_->analyticalSolution(scv.dofPosition())[Indices::pressureIdx];
                analyticalConcentration_[scv.dofIndex()] = problem_->analyticalSolution(scv.dofPosition())[Indices::conti0EqIdx + 1];

                for (int dirIdx = 0; dirIdx < dimWorld; ++dirIdx)
                    analyticalVelocity_[scv.dofIndex()][dirIdx] = problem_->analyticalSolution(scv.center())[Indices::velocity(dirIdx)];
            }
        }
    }
