...
for line in logfile:
    ...
    errorK.append(float(line[11]))
    errorW.append(float(line[14]))
...
resultsK = []
resultsW = []
logfile.truncate(600)
logfile.write("n\terrorP\t\trateP\t\terrorVx\t\trateVx\t\terrorVy\t\trateVy\t\terrorK\t\trateK\t\terrorW\t\trateW\n")
logfile.write("-"*50 + "\n")
for i in range(len(errorP)-1):
    if isnan(errorP[i]) or isinf(errorP[i]):
        continue
    if not ((errorP[i] < 1e-12 or errorP[i+1] < 1e-12) and (errorVx[i] < 1e-12 or errorVx[i+1] < 1e-12) and (errorVy[i] < 1e-12 or errorVy[i+1] < 1e-12)and (errorK[i] < 1e-12 or errorK[i+1] < 1e-12) and (errorW[i] < 1e-12 or errorW[i+1] < 1e-12)):
        ...
        rateK = (log(errorK[i])-log(errorK[i+1]))/log(2)
        rateW = (log(errorW[i])-log(errorW[i+1]))/log(2)
        message = "{}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\t{:0.4e}\n".format(i, errorP[i], rateP,  errorVx[i], rateVx, errorVy[i], rateVy, errorK[i], rateK, errorW[i], rateW)
        logfile.write(message)
        resultsP.append(rateP)
        resultsVx.append(rateVx)
        resultsVy.append(rateVy)
        resultsK.append(rateK)
        resultsW.append(rateW)
    else:
        logfile.write("error: exact solution!?")
i = len(errorP)-1
message = "{}\t{:0.4e}\t\t{}\t{:0.4e}\t\t{}\t{:0.4e}\t\t{}\t{:0.4e}\t\t{}\t{:0.4e}\n".format(i, errorP[i], "",  errorVx[i], "", errorVy[i], "", errorK[i], "", errorW[i], "")
logfile.write(message)
...
# check the rates, we expect rates around 2
if mean(resultsK) < 2.05 and mean(resultsK) < 1.95:
    sys.stderr.write("*"*70 + "\n" + "The convergence rates for tke were not close enough to 2! Test failed.\n" + "*"*70 + "\n")
    sys.exit(1)
# check the rates, we expect rates around 2
if mean(resultsW) < 2.05 and mean(resultsW) < 1.95:
    sys.stderr.write("*"*70 + "\n" + "The convergence rates for omega were not close enough to 2! Test failed.\n" + "*"*70 + "\n")
    sys.exit(1)
