    ...
    static auto calculateL2Error(const Problem& problem, const SolutionVector& curSol)
    {
        ...
        for (const auto& element : elements(problem.gridGeometry().gridView()))
        {
            ...
            for (auto&& scv : scvs(fvGeometry))
            {
                ...
                const auto analyticalSolutionCellCenterP = problem.getAnalyticalPressureSolution()[scv.dofIndex()];
                const auto analyticalSolutionCellCenterK = problem.getAnalyticalTKESolution()[scv.dofIndex()];
                const auto analyticalSolutionCellCenterW = problem.getAnalyticalDissipationSolution()[scv.dofIndex()];
                const auto numericalSolutionCellCenterP = curSol[GridGeometry::cellCenterIdx()][dofIdxCellCenter][Indices::pressureIdx - ModelTraits::dim()];
                const auto numericalSolutionCellCenterK = curSol[GridGeometry::cellCenterIdx()][dofIdxCellCenter][Indices::turbulentKineticEnergyIdx - ModelTraits::dim()];
                const auto numericalSolutionCellCenterW = curSol[GridGeometry::cellCenterIdx()][dofIdxCellCenter][Indices::dissipationIdx - ModelTraits::dim()];
                sumError[Indices::pressureIdx] += squaredDiff_(analyticalSolutionCellCenterP, numericalSolutionCellCenterP) * Extrusion::volume(scv);
                sumError[Indices::turbulentKineticEnergyIdx] += squaredDiff_(analyticalSolutionCellCenterK, numericalSolutionCellCenterK) * Extrusion::volume(scv);
                sumError[Indices::dissipationIdx] += squaredDiff_(analyticalSolutionCellCenterW, numericalSolutionCellCenterW) * Extrusion::volume(scv);
                sumReference[Indices::pressureIdx] += analyticalSolutionCellCenterP * analyticalSolutionCellCenterP * Extrusion::volume(scv);
                sumReference[Indices::turbulentKineticEnergyIdx] += analyticalSolutionCellCenterK * analyticalSolutionCellCenterK * Extrusion::volume(scv);
                sumReference[Indices::dissipationIdx] += analyticalSolutionCellCenterW * analyticalSolutionCellCenterW * Extrusion::volume(scv);
                ...
                for (auto&& scvf : scvfs(fvGeometry))
                {
                    ...
                    const auto analyticalSolutionFace = problem.getAnalyticalVelocitySolutionOnFace()[scvf.dofIndex()][Indices::velocity(dirIdx)];
        ...
        // get the absolute and relative discrete L2-error for cell-center dofs
        l2NormAbs[Indices::turbulentKineticEnergyIdx] = std::sqrt(sumError[Indices::turbulentKineticEnergyIdx] / totalVolume);
        l2NormRel[Indices::turbulentKineticEnergyIdx] = std::sqrt(sumError[Indices::turbulentKineticEnergyIdx] / sumReference[Indices::turbulentKineticEnergyIdx]);

        // get the absolute and relative discrete L2-error for cell-center dofs
        l2NormAbs[Indices::dissipationIdx] = std::sqrt(sumError[Indices::dissipationIdx] / totalVolume);
        l2NormRel[Indices::dissipationIdx] = std::sqrt(sumError[Indices::dissipationIdx] / sumReference[Indices::dissipationIdx]);
