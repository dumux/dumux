...
int main(int argc, char** argv)
{
    ...
    using MomentumTypeTag = Properties::TTag::RANSLowReKEpsilonChannelTestMomentum;
    using MassTypeTag = Properties::TTag::RANSLowReKEpsilonChannelTestMass;
    ...
    // dumux/freeflow/rans/lowrekepsilon/massproblem.hh
    ...
    // immediately triggers LowReKEpsilonMassVolumeVariables::update() for every element, which
    // reads momentumProblem->wallDistance(eIdx)/stressTensorScalarProduct(eIdx)/yPlus(eIdx): an
    // out-of-bounds read on the still-empty momentum-side bookkeeping vectors if this were
    // called after (see the one-equation test's main.cc for the same ordering bug/fix).
