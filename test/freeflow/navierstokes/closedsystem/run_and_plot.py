...
for re in reynolds:
    if not args['skipsim']:
        subprocess.run(['make', 'test_ff_navierstokes_closedsystem_new'], check=True)
        subprocess.run(['./test_ff_navierstokes_closedsystem_new', 'params_re' + str(re) + '.input'], check=True)
