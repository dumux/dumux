...
class DoneaTestProblemNewInterface : public BaseProblem
{
    ...
    using ConstraintInfo = typename ParentType::ConstraintInfo;
    using ConstraintValues = typename ParentType::ConstraintValues;
...
public:
    ...
    /*!
     * \name Boundary conditions (constraints)
     */
    // \{
    ...
    /*!
     * \brief Specifies which primary variable is set as constraint (dirichlet)
     *        and which equation is replaced by it.
     *
    ...
    ConstraintInfo constraintInfo(const FVElementGeometry& fvGeometry, const LocalDof& localDof) const
    {
        ...
        if constexpr (ParentType::isMomentumProblem())
        {
            const auto& globalPos = fvGeometry.dofPosition(localDof);
            // We set constraints (for both velocity components) on all non-flux boundaries
            if (!onFluxBoundary_(globalPos) && fvGeometry.gridGeometry().dofOnBoundary(localDof.dofIndex()))
                info.setAll();
        }
        else
        {
            // for mass problem we specify some pressure as constraint
            // here simply at boundaries dofs
            if (!useNeumann_ && fvGeometry.gridGeometry().dofOnBoundary(localDof.dofIndex()))
    ...
    /*!
     * \brief The values given for a constraint related to a given position
     *
     * \param globalPos The position of a local dof
     */
    ConstraintValues constraintAtPos(const GlobalPosition& globalPos) const
                                ...
                                const FaceIpData& faceIpData) const
    {
        ...
        if constexpr (ParentType::isMomentumProblem())
        {
            ...
            {
                if( (intersection.centerUnitOuterNormal() - faceIpData.unitOuterNormal()).two_norm() > 1e-8 )
                    continue;

                for (const auto& localDof : localDofs(fvGeometry, intersection))
                    if(onFluxBoundary_(fvGeometry.dofPosition(localDof)))
                        addFlux = true;
            }
     ...
     * \param globalPos The global position
     */
    ConstraintValues analyticalSolution(const GlobalPosition& globalPos, Scalar time = 0.0) const
    {
...
private:
    ...
    bool onFluxBoundary_(const GlobalPosition& globalPos) const
    {
        static constexpr Scalar eps = 1e-8;
        if (useNeumann_)
            return (globalPos[0] > this->gridGeometry().bBoxMax()[0] - eps) || (globalPos[1] > this->gridGeometry().bBoxMax()[1] - eps);
        else
            return false;
    }
