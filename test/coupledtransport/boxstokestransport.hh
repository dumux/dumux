#ifndef DUNE_BOXSTOKESTRANSPORT_HH
#define DUNE_BOXSTOKESTRANSPORT_HH
...
public:
    ...
    BoxStokesTransport(const Grid& grid, ProblemType& prob)
        : NonlinearModel(grid, prob), uOldTimeStep(grid)
    { }

    BoxStokesTransport(const Grid& grid, ProblemType& prob, int level)
...
{
    enum {velocityXIdx=0, velocityYIdx=1, massFracIdx=dim, pressureIdx=dim+1};
    enum {numEq = dim+2};
public:
                ...
                for (int i = 0; i < size; i++)
                    {
                        ...
                        int globalId = vertexmapper.map(element, sfs[i].entity(), dim);
...
protected:
    ...
    BlockVector<FieldVector<Scalar, 1> > xWN;
