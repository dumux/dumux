// $Id: box2p2c.hh 1555 2009-04-06 16:35:57Z klaus $

#ifndef DUNE_BOXDARCYTRANSPORT_HH
#define DUNE_BOXDARCYTRANSPORT_HH
    ...
    for (ElementIterator eIt = grid.template leafbegin<0>(); eIt != endEIt; ++eIt)
        {
            ...
            FieldMatrix<Scalar, dim, dim> K = problem.soil().K(global,element,local);
...
class BoxDarcyTransport
    : public LeafP1TwoPhaseModel<GridT, ScalarT, TwoPTwoCProblem<GridT, ScalarT>, BoxDarcyTransportJacobian<GridT, ScalarT> >
{
public:
    ...
    typedef BoxDarcyTransportJacobian<Grid, Scalar> LocalJacobian;
    typedef LeafP1TwoPhaseModel<Grid, Scalar, ProblemType, LocalJacobian> ThisLeafP1TwoPhaseModel;
    typedef BoxDarcyTransport<Grid, Scalar, VtkMultiWriter> ThisType;
    ...
    enum{m = 2};
    enum {pressureIdx = 0, switchIdx = 1, numberOfComponents = 2};
    ...
    BoxDarcyTransport(const Grid& grid, ProblemType& prob)
    ...
    void initial()
    {
        ...
        std::cout << "initializing Darcy solution." << std::endl;
        ...
        for (Iterator it = gridview.template begin<0>(); it
                 != eendit; ++it)
            {
                ...
                for (int i = 0; i < size; i++)
                    {
                        // get cell center in reference element and global coordinate
                    ...
                    if (is->boundary())
                        {
                            for (int i = 0; i < size; i++)
                                ...
                                for (int j = 0; j < ReferenceElements<Scalar,dim>::general(gt).size(is->indexInInside(), 1, sfs[i].codim()); j++)
                                    if (sfs[i].entity()
                                        == ReferenceElements<Scalar,dim>::general(gt).subEntity(is->indexInInside(), 1,
                                                                                                j, sfs[i].codim()))
                                        {
                                                    ...
                                                    if (this->localJacobian().bc(i)[equationNumber]
                                                        == BoundaryConditions::dirichlet)
                                                        {
                                                            ...
                                                            if (bctype[equationNumber] == BoundaryConditions::dirichlet)
                                                                {
                                                                    FieldVector<Scalar,m> ghelp = this->problem.dirichlet(global, entity, is,local);
    ...
    void vtkout (const char* name, int k)
    {
        ...
        // determine velocities at elements
        BlockVector<FieldVector<Scalar, 1> > xVelocity(this->grid_.size(0));
        BlockVector<FieldVector<Scalar, 1> > yVelocity(this->grid_.size(0));
        calculateDarcyVelocity(this->grid_, this->problem, this->u, xVelocity, yVelocity);

        BlockVector<FieldVector<Scalar, 1> > xWN(this->size);
//        BlockVector<FieldVector<Scalar, 1> > xAW(this->size);
        for (int i = 0; i < this->size; i++)
            {
                this->pW[i] = (*(this->u))[i][pressureIdx];
                xWN[i] = (*(this->u))[i][switchIdx];
        ...
        vtkwriter.addVertexData(xWN,"Xwn");
