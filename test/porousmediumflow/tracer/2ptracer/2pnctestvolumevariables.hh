 ...
 */
#ifndef DUMUX_2PNCTEST_VOLUME_VARIABLES_HH
#define DUMUX_2PNCTEST_VOLUME_VARIABLES_HH
...
struct TwoPNCTestVolumeVariablesTraits
{
...
class TwoPNCTestVolumeVariables
: public PorousMediumFlowVolumeVariables<Traits>
, public EnergyVolumeVariables<Traits, TwoPNCTestVolumeVariables<Traits> >
{
    using ParentType = PorousMediumFlowVolumeVariables<Traits>;
    using EnergyVolVars = EnergyVolumeVariables<Traits, TwoPNCTestVolumeVariables<Traits> >;
...
public:
    ...
    */
                            ...
                            SolidState& solidState)
    {
        ...
        {
            //we know for our case that we only have immiscible the phases water and NAPL and NaCl (other salts) in the water phase.
            Dune::FieldVector<Scalar, ModelTraits::numComponents()> moleFrac;

            moleFrac[comp1Idx] = 0; //no NAPL in water.
            ...
            moleFrac[comp0Idx] = 1 - sumMoleFracOtherComponents; //water mole fraction = 1- all the NaCl &Co

            // convert mass to mole fractions and set the fluid state
            for (int compIdx = 0; compIdx < ModelTraits::numComponents(); ++compIdx)
            {
                //setting the mole fractions from above
                fluidState.setMoleFraction(phase0Idx, compIdx, moleFrac[compIdx]);

                //nothing but NAPL in the non-aquous phase
                fluidState.setMoleFraction(phase1Idx, compIdx, 0);
            }
            //nothing but NAPL in the non-aquous phase
            fluidState.setMoleFraction(phase1Idx, comp1Idx, 1);

        }
        else if (phasePresence == secondPhaseOnly) //only NAPL
        {
            // convert mass to mole fractions and set the fluid state
            for (int compIdx = 0; compIdx < ModelTraits::numComponents(); ++compIdx)
            {
                //setting the mole fractions from above
                fluidState.setMoleFraction(phase0Idx, compIdx, 0);
        ...
        else if (phasePresence == firstPhaseOnly) // only aqueous phase
        {
