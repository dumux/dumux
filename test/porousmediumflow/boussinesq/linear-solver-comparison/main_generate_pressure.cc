...
/*!
 * \file
 * \brief Matrix-generation stage for the pressure-based Boussinesq
 *        linear-solver benchmark.
 *
 * Runs the same transient one-sided Rayleigh-Bénard setup as
 * white-noise-perturbations/main_pressure.cc (perturbed erfc initial
 * condition, production AMG-BiCGSTAB solver) from t = tStart to
 * TimeLoop.TEnd, but instead of writing VTK/diagnostics output it exports the
 * assembled Jacobian and residual at each time in MatrixExport.Times (default
 * 1, 2, 3, 4) to MatrixMarket files (see matrixio.hh for the naming
 * convention).
 *
 * These files are the input to main_solve_pressure.cc, which benchmarks
...
int main(int argc, char** argv)
{
    ...
    // to white-noise-perturbations/main_pressure.cc, so the assembled
    ...
    // white-noise-perturbations/main_pressure.cc. This is *not* one of the
    // solvers benchmarked by main_solve_pressure.cc; it just needs to be
    ...
    if (mpiHelper.rank() == 0)
    {
        std::cout << "Generating pressure-formulation matrices at t = ";
