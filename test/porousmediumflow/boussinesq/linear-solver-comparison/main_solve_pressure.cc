...
/*!
 * \file
 * \brief Matrix-solving stage for the pressure-based Boussinesq
 *        linear-solver benchmark.
 *
 * Reads the Jacobian/residual pairs written by main_generate_pressure.cc (one
 * per time in MatrixExport.Times) from MatrixMarket files and benchmarks each
 * candidate ISTL solver against them (see solvercompare.hh). No grid, problem
 * or transient simulation is set up here -- solving is fully decoupled from
 * how the matrices were produced.
 */
#include <config.h>
...
int main(int argc, char** argv)
{
    ...
    for (const auto t : exportTimes)
    {
        ...
        printSolverComparison("Pressure formulation -- " + state, results);
        writeSolverComparisonCsv(csv, "pressure", results);
    }
