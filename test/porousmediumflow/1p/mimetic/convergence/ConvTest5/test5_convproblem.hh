 ...
 */
#ifndef DUMUX_TEST5_CONV_PROBLEM_HH
#define DUMUX_TEST5_CONV_PROBLEM_HH
...
#include "test5_convspatialparams.hh"

namespace Dumux
{
template <class TypeTag>
class Test5ConvProblem;

namespace Capabilities
{
    template<class TypeTag>
    struct isStationary<Test5ConvProblem<TypeTag>>
...
{
#if PROBLEM==1
NEW_TYPE_TAG(Test5ConvProblem, INHERITS_FROM(OnePMimetic));
SET_TYPE_PROP(Test5ConvProblem, IntersectionMapper, Dumux::NonConformingGridIntersectionMapper<TypeTag>);
#else
NEW_TYPE_TAG(Test5ConvProblem, INHERITS_FROM(CCMpfaModel, OneP));

SET_PROP(Test5ConvProblem, MpfaMethod)
{
...
SET_PROP(Test5ConvProblem, Fluid)
{
private:
...
// Set the grid type
SET_TYPE_PROP(Test5ConvProblem, Grid, Dune::ALUGrid<3, 3, Dune::cube, Dune::nonconforming>);

// Set the problem property
SET_TYPE_PROP(Test5ConvProblem, Problem, Dumux::Test5ConvProblem<TypeTag> );

// Set the spatial parameters
SET_TYPE_PROP(Test5ConvProblem, SpatialParams, Dumux::Test5ConvSpatialParams<TypeTag> );


SET_BOOL_PROP(Test5ConvProblem, EnableGlobalFVGeometryCache, true);

SET_BOOL_PROP(Test5ConvProblem, EnableGlobalFluxVariablesCache, true);
SET_BOOL_PROP(Test5ConvProblem, EnableGlobalVolumeVariablesCache, true);

// Enable gravity
SET_BOOL_PROP(Test5ConvProblem, ProblemEnableGravity, false);

SET_TYPE_PROP(Test5ConvProblem, LinearSolver, SuperLUBackend<TypeTag> );

#if PROBLEM==1
// The geometry helper required for the stencils, etc.
SET_PROP(Test5ConvProblem, StaggeredGeometryHelper)
{
private:
...
template <class TypeTag>
class Test5ConvProblem : public ImplicitPorousMediaProblem<TypeTag>
{
...
public:
    Test5ConvProblem(TimeManager &timeManager, const GridView &gridView)
                ...
                const SubControlVolume &scv) const
    {
        ...
        for(; qp != qend; ++qp)
        {
            ...
            // x and y part
            Scalar val =  sin(pi_*z)*
                          (pi_*(x*cos(pi_*x)*sin(pi_*y) + y*cos(pi_*y)*sin(pi_*x) -
            ...
            // z part
            val -= 2.0*z*pi_*cos(pi_*z)*sin(pi_*x)*sin(pi_*y);
            val += (z*z + alpha)*pi_*pi_* sin(pi_*z)*sin(pi_*x)*sin(pi_*y);
    ...
    Scalar exact (const GlobalPosition& globalPos) const
    {
        ...
        return sin(pi_*x)*sin(pi_*y)*sin(pi_*z) + 1.0;
    ...
    Dune::FieldVector<Scalar,dim> exactGrad (const GlobalPosition& globalPos, const GlobalPosition& regionPos) const
    {
        ...
        grad[0] = pi_*cos(pi_*x)*sin(pi_*y)*sin(pi_*z);
        grad[1] = pi_*cos(pi_*y)*sin(pi_*x)*sin(pi_*z);
        grad[2] = pi_*cos(pi_*z)*sin(pi_*x)*sin(pi_*y);
        return grad;
    }
