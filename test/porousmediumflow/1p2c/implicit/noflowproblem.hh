 ...
 *****************************************************************************/
#ifndef DUMUX_1P2CNI_NOFLOW_PROBLEM_HH
#define DUMUX_1P2CNI_NOFLOW_PROBLEM_HH
...
{
NEW_TYPE_TAG(NoFlowProblem, INHERITS_FROM(OnePTwoCNI));
...
SET_TYPE_PROP(NoFlowProblem, SpatialParams, OnePTwoCNoflowSpatialParams<TypeTag>);
...
public:
    ...
    void postTimeStep() const
    {
        ...
        Scalar data3 = this->gridView().comm().sum(advectiveCompFluxInFracture);
        Scalar data4 = this->gridView().comm().sum(diffusiveCompFluxInFracture);
        Scalar data5 = this->gridView().comm().sum(heatFluxInFracture);
        Scalar data6 = this->gridView().comm().sum(kgradpFromFracture);
        Scalar data7 = this->gridView().comm().sum(phaseFluxFromFracture);
        Scalar data8 = this->gridView().comm().sum(advectiveCompFluxFromFracture);
        Scalar data9 = this->gridView().comm().sum(diffusiveCompFluxFromFracture);
        Scalar data10 = this->gridView().comm().sum(heatFluxFromFracture);
