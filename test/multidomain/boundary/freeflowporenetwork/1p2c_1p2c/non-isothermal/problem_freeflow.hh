...
class FreeFlowOnePNCNITestProblem :  public BaseProblem
{
...
public:
    FreeFlowOnePNCNITestProblem(std::shared_ptr<const GridGeometry> gridGeometry, std::shared_ptr<CouplingManager> couplingManager)
                                ...
                                const SubControlVolumeFace& scvf) const
    {
        ...
        else
        {
            if (couplingManager_->isCoupled(CouplingManager::freeFlowMassIndex, CouplingManager::poreNetworkIndex, scvf))
                ...
                values.setAllNeumann(); //outflow or zero flux BCs for p,T,x
        }
        return values;
    }
