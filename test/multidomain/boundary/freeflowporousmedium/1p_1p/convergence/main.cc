 ...
 * Returns a tuple of the analytical solution for the velocity at the faces
 * \param problem the problem for which to evaluate the analytical solution
 */
template<class Scalar, class Problem>
auto createFaceCenterAnalyticalSolution(const Problem& problem)
{
    const auto& gridGeometry = problem.gridGeometry();
    std::vector<Scalar> analyticalVelocityOnFace(gridGeometry.numDofs());
    ...
    for (const auto& element : elements(gridGeometry.gridView()))
    {
        ...
        for (const auto& scv : scvs(fvGeometry))
        {
            ...
            const auto faceDofPosition = scv.center();
            const auto dirIdx = scv.dofAxis();
    ...
    return analyticalVelocityOnFace;
}
...
*/
template<class Scalar, class Problem>
auto createCellCenterAnalyticalSolution(const Problem& problem)
{
    const auto& gridGeometry = problem.gridGeometry();
...
template<class MassP, class MomP, class MassV, class MomV>
void printFreeFlowL2Error(const MassP& massProblem, const MomP& momentumProblem,
                          const MassV& massSol, const MomV& momSol)
{
    using namespace Dumux;

    const auto [l2errorAbsVelocity, l2errorRelVelocity] = calculateL2Error(momentumProblem, momSol);
    const auto [l2errorAbsPressure, l2errorRelPressure] = calculateL2Error(massProblem, massSol);
    ...
    std::cout << Fmt::format("** L2 error (abs/rel) for {} cc dofs and {} face dofs (total: {}): ",
              ...
              << Fmt::format("L2(p) = {:.8e} / {:.8e}", l2errorAbsPressure[0], l2errorRelPressure[0])
              << Fmt::format(", L2(vx) = {:.8e} / {:.8e}", l2errorAbsVelocity[0], l2errorRelVelocity[0])
              << Fmt::format(", L2(vy) = {:.8e} / {:.8e}", l2errorAbsVelocity[1], l2errorRelVelocity[1])
              << std::endl;

    // write the norm into a log file
    std::ofstream logFile(massProblem.name() + ".log", std::ios::app);
    logFile << "[ConvergenceTest] L2(p) = " << l2errorAbsPressure[0]
            << " L2(vx) = " << l2errorAbsVelocity[0]
            << " L2(vy) = " << l2errorAbsVelocity[1]
            << std::endl;
}
...
int main(int argc, char** argv)
{
    ...
    auto freeFlowMomentumGridGeometry = std::make_shared<FreeFlowMomentumFVGridGeometry>(freeFlowGridView);
    freeFlowMomentumGridGeometry->update();
    using FreeFlowMassFVGridGeometry = GetPropType<FreeFlowMassTypeTag, Properties::GridGeometry>;
    auto freeFlowMassGridGeometry = std::make_shared<FreeFlowMassFVGridGeometry>(freeFlowGridView);
    freeFlowMassGridGeometry->update();
    ...
    const auto freeFlowAnalyticalSolutionCC = createCellCenterAnalyticalSolution<double>(*freeFlowMassProblem);
    freeflowVtkWriter.addField(std::get<0>(freeFlowAnalyticalSolutionCC), "pressureExact");
    freeflowVtkWriter.addField(std::get<1>(freeFlowAnalyticalSolutionCC), "velocityExact");
    ...
    const auto darcyAnalyticalSolution = createCellCenterAnalyticalSolution<double>(*darcyProblem);
    ...
    std::cout << "NumDofs Face " << freeFlowMomentumGridGeometry->numDofs() << std::endl;
    std::cout << "NumDofs CC " << freeFlowMassGridGeometry->numDofs() << std::endl;
    std::cout << "NumDofs Darcy " << darcyGridGeometry->numDofs() << std::endl;
    ...
    printFreeFlowL2Error(*freeFlowMassProblem, *freeFlowMomentumProblem,
                         sol[freeFlowMassIndex], sol[freeFlowMomentumIndex]);
