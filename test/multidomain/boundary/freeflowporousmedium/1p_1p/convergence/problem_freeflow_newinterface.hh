...
public:
    ...
    FreeFlowSubProblemNewInterface(std::shared_ptr<const GridDiscretization> gridDiscretization,
                                   std::shared_ptr<CouplingManager> couplingManager,
                                   const DarcyStokesTestCase testCase,
                                   const std::string& name)
    : ParentType(gridDiscretization, couplingManager, "FreeFlow")
                                ...
                                const FaceIpData& faceIpData) const
    {
        ...
        if constexpr (ParentType::isMomentumProblem())
        {
            ...
            if (isCoupled_(CouplingManager::freeFlowMomentumIndex, CouplingManager::porousMediumIndex, elemDisc, faceIpData)
                && globalPos[0] > this->gridDiscretization().bBoxMin()[0] + eps_
                && globalPos[0] < this->gridDiscretization().bBoxMax()[0] - eps_)
            {
                ...
                const auto v = evalSolution(element, element.geometry(), elemDisc.gridDiscretization(), elemSol, globalPos);
