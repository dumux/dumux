 ...
 * \brief The problem for the darcy domain in the single-phase stokes-darcy mortar-coupling test
 */
#ifndef DUMUX_MORTAR_STOKES_DARCY_ONEP_TEST_DARCY_PROBLEM_HH
#define DUMUX_MORTAR_STOKES_DARCY_ONEP_TEST_DARCY_PROBLEM_HH
...
public:
    DarcyProblem(std::shared_ptr<const GridGeometry> gridGeometry, const std::string& paramGroup = "")
                                ...
                                const SolutionVector& x) const
    {
        return this->getTraceOperator(mortarId).assembleScvfVariables(
            [&] (const auto& scvf, const auto& context) -> Dune::FieldVector<double, 1> {
                ...
                return {fluxVars.advectiveFlux(0, [] (const auto& vv) { return vv.mobility(0); })/scvf.area()};
            },
            FVContext<GridVariables, SolutionVector>{gridVariables, x}
        );
    }
    ...
    BoundaryTypes boundaryTypes(const Element& element, const SubControlEntity& sce) const
    {
        ...
        else
        {
            if constexpr (isCVFE)
            {
                if (onBottomBoundary_(sce.dofPosition()))
                    values.setAllDirichlet();
            }
            else
            {
                if (onBottomBoundary_(sce.ipGlobal()))
                    values.setAllDirichlet();
            }
        }
        return values;
    ...
    PrimaryVariables dirichlet(const Element& element, const SubControlEntity& sce) const
    {
        ...
        if constexpr (isCVFE)
            return PrimaryVariables(bottomPressure_(sce.dofPosition()));
        else
            return PrimaryVariables(bottomPressure_(sce.ipGlobal()));
    }
...
private:
    ...
    double bottomPressure_(const GlobalPosition& globalPos) const
    { return 1.0; }
