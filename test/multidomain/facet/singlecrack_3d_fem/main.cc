...
int main(int argc, char** argv) try
{
    ...
    std::vector<double> aperture(lagrangeGridView.size(0));
    std::vector<Dune::FieldVector<double, 3>> deltaUt(lagrangeGridView.size(0));
    std::vector<Dune::FieldVector<double, 3>> deltaUn(lagrangeGridView.size(0));

    const double initialAperture = getParam<double>("OnePFacet.SpatialParams.InitialAperture");
    Dune::FieldVector<double, 3> e1({1.0, 0.0, 0.0});
    ...
    for (const auto& element : elements(lagrangeGridView))
    {
        ...
        deltaUt[eIdx] = couplingManager->computeTangentialDisplacementJump(element, eg.center());
        deltaUn[eIdx] = contactSurface.getBasisVector(2);
        deltaUn[eIdx] *= couplingManager->computeNormalDisplacementJump(element, eg.center());

        if (contactSurface.getBasisVector(2)*e1 < 0.0)
        {
        ...
        aperture[eIdx] = couplingManager->computeAperture(element, eg.center(), initialAperture);
        fricCoeff[eIdx] = problem[lagrangeId].frictionCoefficientAtPos(eg.center());
    }

    auto gf_traction = Dune::Functions::makeDiscreteGlobalBasisFunction<Dune::FieldVector<double, 3>>(*lagrangeBasis, x_transform);
    auto gf_deltaUt = Dune::Functions::makeDiscreteGlobalBasisFunction<Dune::FieldVector<double, 3>>(*lagrangeBasis, deltaUt);
    auto gf_deltaUn = Dune::Functions::makeDiscreteGlobalBasisFunction<Dune::FieldVector<double, 3>>(*lagrangeBasis, deltaUn);
    lagrangeWriter.addCellData(gf_traction, Dune::VTK::FieldInfo("lagrange", Dune::VTK::FieldInfo::Type::vector, 3));
    lagrangeWriter.addCellData(gf_deltaUt, Dune::VTK::FieldInfo("deltaUt", Dune::VTK::FieldInfo::Type::vector, 3));
    lagrangeWriter.addCellData(gf_deltaUn, Dune::VTK::FieldInfo("deltaUn", Dune::VTK::FieldInfo::Type::vector, 3));
    lagrangeWriter.addCellData(fricCoeff, "fricCoeff");
    lagrangeWriter.addCellData(aperture, "aperture");
    lagrangeWriter.write("lagrange");

    // write out displacements
    Dune::VTKWriter<MechanicalGridView> mechWriter(mechGridView);
    auto u = Dune::Functions::makeDiscreteGlobalBasisFunction<Dune::FieldVector<double, 3>>(*mechSpaceBasis, x[bulkMechId]);
    mechWriter.addVertexData(u, Dune::VTK::FieldInfo("u", Dune::VTK::FieldInfo::Type::vector, 3));
    ...
    auto sigmaX = std::make_shared<P0Function>(mechGridView, fvGridGeometry[bulkMechId].elementMapper(), problem[bulkMechId].sigma_x(), "Sigma_x", 3);
    auto sigmaY = std::make_shared<P0Function>(mechGridView, fvGridGeometry[bulkMechId].elementMapper(), problem[bulkMechId].sigma_y(), "Sigma_y", 3);
    auto sigmaZ = std::make_shared<P0Function>(mechGridView, fvGridGeometry[bulkMechId].elementMapper(), problem[bulkMechId].sigma_z(), "Sigma_z", 3);

    mechWriter.addCellData(sigmaX);
    mechWriter.addCellData(sigmaY);
    mechWriter.addCellData(sigmaZ);
