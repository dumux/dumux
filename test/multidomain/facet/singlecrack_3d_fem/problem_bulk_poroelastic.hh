...
// Set the grid type
template<class TypeTag>
struct Grid<TypeTag, TTag::PoroElasticBulk> { using type = Dune::ALUGrid<3, 3, Dune::simplex, Dune::nonconforming>; };
...
public:
    ...
    , couplingManagerPtr_(couplingManagerPtr)
    {
        ...
        sigma_z_.resize(gridGeometry->gridView().size(0), Force(0.0));
    }
    ...
    BoundaryTypes boundaryTypesAtPos(const GlobalPosition& globalPos) const
    {
        ...
        // this assumes quadratic bottom boundaries
        ...
        static const auto distThreshold = 0.2*0.5*dx;

        if (globalPos[2] < this->gridGeometry().bBoxMin()[2] + 1e-6)
        {
            values.setDirichlet(Indices::uzIdx);

            const auto x = globalPos[0] - c;
            const auto y = globalPos[1] - c;
                            ...
                            Id id)
    {
        for (const auto& element : elements(this->gridGeometry().gridView()))
        {
            ...
            sigma_z_[eIdx] = sigma[Indices::uzIdx];
        }
    }
    ...
    const std::vector<Force>& sigma_z() const { return sigma_z_; }
