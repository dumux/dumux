...
int main(int argc, char** argv) try
{
    ...
    {
        deltaUt[fvGridGeometry[lagrangeId].elementMapper().index(element)] = couplingManager->computeTangentialDisplacementJump(element, element.geometry().center())[1];
        deltaUn[fvGridGeometry[lagrangeId].elementMapper().index(element)] = couplingManager->computeNormalDisplacementJump(element, element.geometry().center());
        fricCoeff[fvGridGeometry[lagrangeId].elementMapper().index(element)] = problem[lagrangeId].frictionCoefficientAtPos(element.geometry().center());
    }
