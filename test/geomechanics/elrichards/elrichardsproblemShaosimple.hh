...
#include "elrichardsspatialparamsShao1Layer.hh"
     ...
     for (int j=0; j<n_ + 1; ++j)
       {
         ...
         stress[0] =pnRef_+((1 - porosity) * rockDensity * gravity * ( zMax(globalPos)- globalPos[dim-1])) + (waterDensity_*9.81*porosity* swnew* ( zMax(globalPos)- globalPos[dim-1]));//in shao23degeree pnRef_is not added!!!!!
         if(dim >=2)
          stress[1] = pnRef_+((1 - porosity) * rockDensity * gravity * ( zMax(globalPos)- globalPos[dim-1])) + (waterDensity_*9.81*porosity* swnew* ( zMax(globalPos)- globalPos[dim-1]));
         if(dim == 3)
          stress[2] = pnRef_+((1 - porosity) * rockDensity * gravity * ( zMax(globalPos)- globalPos[dim-1])) + (waterDensity_*9.81*porosity* swnew*( zMax(globalPos)- globalPos[dim-1]));
