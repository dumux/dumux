...
//          depthBOR(globalPos) = yMax(globalPos)-globalPos[1];
    ...
    {
        return  zMax(globalPos) - globalPos[2];;
    }
//     Scalar depth = zMax - globalPos[2];
...
//             Scalar depth = zMax(globalPos) - globalPos[2];
            pInit_[vIdxGlobal] = -pnRef_ + waterDensity_*9.81*depth(globalPos); //pnRef_;//equal to the water pressure and not the pressure in the system which is airpressure.therefore it should be defined from the initial capillary pressure and not the other way round.
        }
    }
...
//                   + (1 - porosity) * rockDensity * gravity * ( - globalPos[dim-1]);
      stress[0] = pnRef_ + (1 - porosity) * rockDensity * gravity * ( - globalPos[dim-1]);
      if(dim >=2)
      stress[1] = pnRef_ + (1 - porosity) * rockDensity * gravity * ( - globalPos[dim-1]);
      if(dim == 3)
      stress[2] = pnRef_ + (1 - porosity) * rockDensity * gravity * ( - globalPos[dim-1]);
...
//             }
        ...
        if (leftBoundary_(globalPos)|| frontBoundary_(globalPos)|| backBoundary_(globalPos)){
...
//                 values.setDirichlet(contiEqIdx);
    ...
    {
        values = 0.0;
//         Scalar depth = 0.0 - globalPos[2];
        values[contiEqIdx] =pnRef_;
...
/*
    ...
    {
        return (onSlopeBoundary_(globalPos)|| topRightBoundary_(globalPos)||topLeftBoundary_(globalPos));
    }

    bool onLowerBoundary_(const GlobalPosition &globalPos) const
    {
        return globalPos[2] < -40. + eps_;
    }

    bool leftBoundary_(const GlobalPosition &globalPos) const //is that small part of the upper boundary
    {
        return (globalPos[0] < -32. + eps_);
    }

    bool rightBoundary_(const GlobalPosition &globalPos) const //is that small part of the upper boundary
    {
        return (globalPos[0] > 35. - eps_);
    }

    bool frontBoundary_(const GlobalPosition &globalPos) const //is that small part of the upper boundary
    {
        return (globalPos[1] > 20. - eps_);
    }

    bool backBoundary_(const GlobalPosition &globalPos) const //is that small part of the upper boundary
    {
        return (globalPos[1] < 0. + eps_);
    }
