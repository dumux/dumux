...
#include <dumux/material/fluidsystems/h2oairrelativepressure.hh>//New
// #include <dumux/material/fluidsystems/h2oair.hh>//OlD
...
SET_TYPE_PROP(ElRichards_TestProblem, Grid, Dune::ALUGrid<2, 2, Dune::cube, Dune::nonconforming>);
...
SET_PROP(ElRichards_TestProblem, WettingPhase)
{
private:
    typedef typename GET_PROP_TYPE(TypeTag, Scalar) Scalar;
public:
...
SET_SCALAR_PROP(ElRichards_TestProblem, NewtonMaxRelativeShift, 1e-1);
...
public:
        ...
        gridView_(gridView)
    {
        std::cout << "ElRichards_TestProblem: Initializing the fluid system for the elrichards model\n";
         ...
         //eps_ = 3e-6;
        pnRef_ = 0; // to adopt Comsol otherwise it is 1e5;
...
//                 pInit_[vIdxGlobal] = -(pnRef_+waterDensity_*9.81*wTdepth(globalPos)); //initial 'water'pressure
    ...
    void setPressure()
    {
        ...
        {
            int vIdxGlobal = this->vertexMapper().index(vertex);
            pInit_[vIdxGlobal] = -this->model().curSol().base()[vIdxGlobal][0];//finds the correct initial pressure distribution after initialiyation and replace it again in Pinit
//             pInit_[vIdxGlobal] = -this->model().curSol().base()[vIdxGlobal][2][0];//this [2]defines the number of primarz variables which is 2 for 2p, and this didnt let the elrichards to converge for the main solution
        }
    }
    ...
    Scalar zMax(const GlobalPosition &globalPos) const
    {
     if (globalPos[0]<6.3+eps_)
          ...
          return (5.-15.)/(23.6-6.3)*(globalPos[0])+ (322.5/17.3);
...
//      Scalar ks_ = GET_RUNTIME_PARAM(TypeTag, Scalar, HydraulicParameters.ks);
        ...
        const Scalar pW = 1e5 + elemVolVars[scvIdx].pressure(wPhaseIdx);
    ...
    inline void evaluateGlobal(const DomainType & position, RangeType & values) const
    {
            ...
            for (const auto& vertex : vertices(gridView_))
            {
                                ...
                                && globalPos[dimWorld-1] <= position[dimWorld-1] + eps_)
                {
                    ...
                    Scalar n_ = GET_RUNTIME_PARAM(TypeTag, Scalar, HydraulicParameters.n);
