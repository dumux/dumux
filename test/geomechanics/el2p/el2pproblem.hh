...
public:
    ...
    void init()
    {
        ...
        angleElement_.resize(numElements);

        ParentType::init();

        calculateElementAngle();
    }
...
//             this->setPressure();
        ...
        if(this->model().anyFailure())
        {
            // re-set the episode to 100 s
            Scalar deltaT= GET_RUNTIME_PARAM(TypeTag, Scalar,FailureParameters.DeltaT);
            this->timeManager().startNextEpisode(deltaT);
            this->timeManager().setTimeStepSize(deltaT);
        }
        else
        {
...
//                 episodeLength_ = 100.00;
    ...
    Scalar getAngleElement(const int eIdx) const
    {
        return angleElement_[eIdx];
    }

    Scalar calculateElementAngle()
    {
        ...
        for (const auto& element : elements(gridView_))
        {
            if(element.partitionType() == Dune::InteriorEntity)
            {
                ...
                for (; isIt != isEndIt; ++isIt)
                {
                    ...
                    // calculate angle between y' and y axis by picking a certain interface. Second part is need for y' = y
                    if( ((outerNormalIntersection[0] > eps_) && (outerNormalIntersection[1] > eps_)) ||
                    ((outerNormalIntersection[0] == 0) && (outerNormalIntersection[1] == 1)) )
                    {
                        angleElement_[eIdx] = atan2(outerNormalIntersection[1],outerNormalIntersection[0])*180/M_PI - 90;
                    }
                }
...
private:
    ...
    std::vector<Scalar> angleElement_;
