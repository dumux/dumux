 ...
 *****************************************************************************/
#ifndef DUMUX_NOFLOWDOMAIN_2P2C_TEST_PROBLEM_HH
#define DUMUX_NOFLOWDOMAIN_2P2C_TEST_PROBLEM_HH
...
NEW_TYPE_TAG(NoFlowDomain2p2cFacetCoupling, INHERITS_FROM(MixedDimensionFacetCoupling));

// the problem property
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, Problem, Dumux::NoFlowDomainFacetCouplingProblem<TypeTag>);

// set the facet coupling grid creator
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, GridCreator, Dumux::GmshDualFacetGridCreator<TypeTag>);

// Set the two sub-problems of the global problem
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, BulkProblemTypeTag, TTAG(NoFlowDomainMatrixProblem));
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, LowDimProblemTypeTag, TTAG(NoFlowDomainFractureProblem));

// The coupling manager
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, CouplingManager, CCMpfaFacetCouplingManager<TypeTag>);

// The linear solver to be used
SET_TYPE_PROP(NoFlowDomain2p2cFacetCoupling, LinearSolver, ILU0BiCGSTABBackend<TypeTag>);

// The sub-problems need to know the global problem's type tag
SET_TYPE_PROP(NoFlowDomainMatrixProblem, GlobalProblemTypeTag, TTAG(NoFlowDomain2p2cFacetCoupling));
SET_TYPE_PROP(NoFlowDomainFractureProblem, GlobalProblemTypeTag, TTAG(NoFlowDomain2p2cFacetCoupling));

// The subproblems inherit the parameter tree from this problem
SET_PROP(NoFlowDomainMatrixProblem, ParameterTree) : GET_PROP(TTAG(NoFlowDomain2p2cFacetCoupling), ParameterTree) {};
SET_PROP(NoFlowDomainFractureProblem, ParameterTree) : GET_PROP(TTAG(NoFlowDomain2p2cFacetCoupling), ParameterTree) {};
