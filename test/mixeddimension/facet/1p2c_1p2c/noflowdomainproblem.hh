 ...
 *****************************************************************************/
#ifndef DUMUX_1P2CNI_NOFLOWDOMAIN_TEST_PROBLEM_HH
#define DUMUX_1P2CNI_NOFLOWDOMAIN_TEST_PROBLEM_HH
...
NEW_TYPE_TAG(NoFlowDomain1p2cniFacetCoupling, INHERITS_FROM(MixedDimensionFacetCoupling));

// the problem property
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, Problem, Dumux::NoFlowDomainFacetCouplingProblem<TypeTag>);

// set the facet coupling grid creator
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, GridCreator, Dumux::GmshDualFacetGridCreator<TypeTag>);

// Set the two sub-problems of the global problem
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, BulkProblemTypeTag, TTAG(NoFlowDomainMatrixProblem));
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, LowDimProblemTypeTag, TTAG(NoFlowDomainFractureProblem));

// The coupling manager
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, CouplingManager, CCMpfaFacetCouplingManager<TypeTag>);

// The linear solver to be used
SET_TYPE_PROP(NoFlowDomain1p2cniFacetCoupling, LinearSolver, ILU0BiCGSTABBackend<TypeTag>);

// The sub-problems need to know the global problem's type tag
SET_TYPE_PROP(NoFlowDomainMatrixProblem, GlobalProblemTypeTag, TTAG(NoFlowDomain1p2cniFacetCoupling));
SET_TYPE_PROP(NoFlowDomainFractureProblem, GlobalProblemTypeTag, TTAG(NoFlowDomain1p2cniFacetCoupling));

// The subproblems inherit the parameter tree from this problem
SET_PROP(NoFlowDomainMatrixProblem, ParameterTree) : GET_PROP(TTAG(NoFlowDomain1p2cniFacetCoupling), ParameterTree) {};
SET_PROP(NoFlowDomainFractureProblem, ParameterTree) : GET_PROP(TTAG(NoFlowDomain1p2cniFacetCoupling), ParameterTree) {};
