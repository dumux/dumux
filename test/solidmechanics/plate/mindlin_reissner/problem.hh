...
//
#ifndef DUMUX_MINDLIN_REISSNER_PLATE_TEST_PROBLEM_HH
#define DUMUX_MINDLIN_REISSNER_PLATE_TEST_PROBLEM_HH
...
class MindlinReissnerPlateTestProblemRotation : public FVProblem<TypeTag>
{
...
public:
    MindlinReissnerPlateTestProblemRotation(std::shared_ptr<const GridGeometry> gridGeometry,
...
template<class TypeTag>
class MindlinReissnerPlateTestProblemDeformation : public FVProblem<TypeTag>
{
    using ParentType = FVProblem<TypeTag>;
    using GridGeometry = GetPropType<TypeTag, Properties::GridGeometry>;
    using FVElementGeometry = typename GridGeometry::LocalView;
...
public:
    MindlinReissnerPlateTestProblemDeformation(std::shared_ptr<const GridGeometry> gridGeometry,
                                              std::shared_ptr<CouplingManager> couplingManager)
    : ParentType(gridGeometry)
    , couplingManager_(couplingManager)
    ...
    , poissonRatio_(getParam<Scalar>("Problem.PoissonRatio"))
    {
        ...
        const auto nu = poissonRatio_;
        stiffness_ = E*t*t*t/(12*(1-nu*nu));
        shearModulus_ = E/(2*(1+nu));
    }

    Scalar thickness() const { return thickness_; }
    Scalar shearCorrectionFactor() const { return 5.0/6.0; }
    Scalar shearModulus() const { return shearModulus_; }
    ...
    // Analytic solution: clamped circular plate under uniform load (Mindlin-Reissner)
    Scalar analyticDeformation(const GlobalPosition& globalPos) const
    {
        ...
        const auto t = this->thickness();
        const auto k = this->shearCorrectionFactor();
        const auto x = r/R;
        const auto xt = t/R;
        return -force_*R*R*R*R/(64*stiffness_)*(1.0-x*x)*(
            (1.0-x*x) + 8*xt*xt/(3*k*(1.0-poissonRatio_))
        );
    }

private:
    std::shared_ptr<CouplingManager> couplingManager_;
    Scalar force_, youngsModulus_, radius_, thickness_, poissonRatio_, stiffness_, shearModulus_;
};
