...
//
// SPP-1748 Cook's membrane — 3D Taylor-Hood (PQ2 displacement + P1 pressure).
...
#include "mixed/properties_taylorhood3d.hh"

int main(int argc, char** argv)
{
    using namespace Dumux;

    using MomTypeTag = Properties::TTag::CooksMembraneTHMomentum3D;
    using PresTypeTag = Properties::TTag::CooksMembraneTHPressure3D;
    using MDTraits = Dumux::Properties::CooksMembraneTHMDTraits3D;
        ...
        if (converged)
        {
            VtkOutputModule<MomGV, typename MDTraits::template SubDomain<0>::SolutionVector>
                vtkWriter(*momGV, x[CouplingManager::momentumIdx],
                          "test_hyperelastic_cooks_membrane_taylorhood3d_lambda"
    ...
    // Note: T2P0 uses P0 (constant) pressure, our TH uses P1 pressure — slightly different.
    const std::vector<Scalar> refsT2   = { 11.37441, -1, -1, -1, -1 };
    const std::vector<Scalar> refsT2P0 = { 11.38386, -1, -1, -1, -1 };

    std::cout << "\n"
              << "SPP-1748 Cook's membrane — 3D Taylor-Hood (PQ2+P1), ψ₁\n"
              << "  T2/T2P0: Galerkin FEM from Table 2.5 (125856 DOFs, λ=432 only).\n"
              << "  Note: T2P0 uses P0 pressure; our TH uses P1 → slightly richer pressure space.\n"
              << std::string(90, '-') << "\n"
              << std::setw(14) << "λ [MPa]"
              << std::setw(10) << "ν"
              << std::setw(14) << "TH3D FVM"
              << std::setw(10) << "/%ref"
              << std::setw(12) << "T2 FEM"
