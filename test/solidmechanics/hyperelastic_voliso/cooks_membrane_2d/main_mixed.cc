...
//
// Cook's membrane benchmark — MINI element (PQ1Bubble displacement + Box/P1 pressure).
// Runs the same λ sweep as the single-field test for direct comparison.
//
#include <config.h>
#include <iomanip>
#include <iostream>
...
#include <dumux/multidomain/fvassembler.hh>  // helpers: CouplingManagerSupportsMultithreadedAssembly etc.
#include <dumux/multidomain/assembler.hh>     // Experimental::MultiDomainAssembler
...
#include "mixed/properties_mixed.hh"

int main(int argc, char** argv)
{
    using namespace Dumux;

    using MomTypeTag = Properties::TTag::CooksMembraneMixedMomentum;
    using PresTypeTag = Properties::TTag::CooksMembraneMixedPressure;
    using MDTraits = Dumux::Properties::CooksMembraneMixedMDTraits;
    using Scalar = typename MDTraits::Scalar;

    Dumux::initialize(argc, argv);
    Parameters::init(argc, argv);

    // grid (shared by both subdomains)
    ...
    // warm start: reuse previous λ solution as initial guess
    for (const Scalar lambda : lambdaValues)
    {
        const Scalar nu = lambda / (2.0*(lambda + mu));

        // update spatial params on both problems
        ...
        if (converged)
        {
            VtkOutputModule<MomGV, typename MDTraits::template SubDomain<0>::SolutionVector>
                vtkWriter(*momGV, x[CouplingManager::momentumIdx],
                          "test_hyperelastic_cooks_membrane_mini_lambda"
    ...
    // 3D p-FEM references from SPP-1748 Table 2.9; see locked test for explanation
    const std::vector<Scalar> refs3D = { 11.381, 11.156, 10.783, 10.746, 10.741 };
    std::cout << "\n"
              << "SPP-1748 Cook's membrane — MINI (PQ1Bubble+P1) mixed formulation, ψ₁\n"
              << "  Improvement over locked (PQ1Bubble single-field) is largest on coarse meshes\n"
              << "  and for nearly-incompressible cases; both converge to the same 2D value (~10.596 mm).\n"
              << std::string(80, '-') << "\n"
              << std::setw(14) << "λ [MPa]"
              << std::setw(10) << "ν"
              << std::setw(18) << "u_y MINI [mm]"
