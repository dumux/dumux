...
//
// Cook's membrane benchmark — Taylor-Hood element (PQ2 displacement + P1 pressure).
// Runs the same λ sweep as the MINI test for direct comparison.
//
#include <config.h>
#include <iomanip>
#include <iostream>
...
#include "mixed/properties_taylorhood.hh"

int main(int argc, char** argv)
{
    using namespace Dumux;

    using MomTypeTag = Properties::TTag::CooksMembraneTHMomentum;
    using PresTypeTag = Properties::TTag::CooksMembraneTHPressure;
    using MDTraits = Dumux::Properties::CooksMembraneTHMDTraits;
        ...
        if (converged)
        {
            VtkOutputModule<MomGV, typename MDTraits::template SubDomain<0>::SolutionVector>
                vtkWriter(*momGV, x[CouplingManager::momentumIdx],
                          "test_hyperelastic_cooks_membrane_taylorhood_lambda"
    ...
    std::cout << "\n"
              << "SPP-1748 Cook's membrane — Taylor-Hood (PQ2+P1) mixed formulation, ψ₁\n"
              << std::string(80, '-') << "\n"
              << std::setw(14) << "λ [MPa]"
              << std::setw(10) << "ν"
              << std::setw(18) << "u_y TH [mm]"
