...
- __Handling dependencies__: Dumux::Dumux module library is linked only against required dune modules and not all of them. The Dune module dependencies are handled then per test target. Downstream Dumux modules require to find and link dependencies if they are not relying on dune_enable_all_packages.
...
- __Geomechanics__: the geomechanics folder is now deprecated and will be removed after release 3.10.
