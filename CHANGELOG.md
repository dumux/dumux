...
- __Grid I/O__: A new writer (grid I/O) and output module (grid I/O + vol var output) - see `dumux/io/gridwriter.hh` - have been added, which build on top of the [GridFormat](https://github.com/dglaeser/gridformat) library. The new writers allow you to write results into various file formats, which can save significant disk space, especially for structured grids. `GridFormat` is pulled via FetchContent. There is also a backup solution via git submodule: GridFormat can be pulled in by `git submodule update --init`.
- __Grid I/O__: The Dumux::VTKReader now uses GridFormat if available. This allows reading many different VTK file formats (including all XML formats with different compression options) and the new VTK-HDF format. Data can also be loaded from all formats.
