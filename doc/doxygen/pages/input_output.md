...
## Container I/O
DuMux supports reading into all sequence containers that provide `begin()`, `end()` and `push_back()` member functions, such as `std::vector<double>` or `std::vector<Dune::FieldVector>`. Writing to file only requires the containers to have `begin()` and `end()` member functions for returning iterators. If you want to read and write simple vectors, have a look at the header `dumux/io/container.hh`. In the following, there are examples of reading in different data formats into DuMux.
...
You can read config data (key-value data) using the `Dune::ParameterTreeParser`. This is the same type of file we use for DuMu<sup>x</sup> `input` files. Given a config file
```ini
[MyData]
...
```
Again, the file extension is arbitrary. You can retrieve data by casting it into another type:
```cpp
...
[XML (Extensible Markup Language)](https://en.wikipedia.org/wiki/XML) is a very flexible structured data format. You can read and write XML files using the [TinyXML-2](http://www.grinninglizard.com/tinyxml2/) library contained in DuMu<sup>x</sup>. For example, given an XML file
```xml
<?xml version="1.0"?>
...
```
you can initialise the file reader via
```cpp
