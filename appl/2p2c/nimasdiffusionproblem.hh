#ifndef DUNE_NIMASDIFFUSIONPROBLEM_HH
#define DUNE_NIMASDIFFUSIONPROBLEM_HH
...
class NimasDiffusionProblem : public BasicDomain<GridT,ScalarT>
{
    typedef GridT                          Grid;
    typedef BasicDomain<Grid, ScalarT>     ParentType;
    typedef NimasDiffusionProblem<Grid, ScalarT>    ThisType;
...
private:
    ...
    enum Episode {
        ConstantRateEpisode,  // an episode with a constant evaporation rate
        FallingRateEpisode,  // an episode with changing evaporation rate
        WaitEpisode    // an episode with diffusion controlled evaporation
...
public:
    NimasDiffusionProblem(Grid *grid,
                 Scalar dtInitial,
                 Scalar tEnd)
        : ParentType(grid),
          ...
          resultWriter_("out_nimasdiffusionproblem")
    {
                   ...
                   int                         boundaryFaceIdx) const
    {
        ...
        if (globalPos[1] > depthBOR_ - eps_)
        {
    		values[switchIdx] = 1e-8; // water in air
        }
    }
...
private:
                  ...
                  const GlobalPosition   &globalPos) const
    {
    	...
    	Scalar initPressure = 1e5;
    	Scalar densityW = wettingPhase().density(temperature, initPressure);
    	Scalar massfrac = 3.5e-3;
//    	massfrac = multicomp().xWN(initPressure, temperature);

		values[pressureIdx] = 1e5 - (depthBOR_ - globalPos[1])*densityW*gravity_[1];
		values[switchIdx] = massfrac; // water in air

		if (globalPos[1] < 0.2)
    	{
    		values[switchIdx] = 0.0238; // residual saturation
    	}
