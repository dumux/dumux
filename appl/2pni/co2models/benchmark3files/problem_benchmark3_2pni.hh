#ifndef PROBLEM_BENCHMARK3_2PNI_HH
#define PROBLEM_BENCHMARK3_2PNI_HH
...
class ProblemBenchmark3_2pni : public TwoPhaseHeatProblem<Grid, RT> {
...
public:
    ...
    virtual FieldVector<RT,m>dirichlet(const FieldVector<Scalar,dim>& x, const Entity& e,
                                 const IntersectionIterator& intersectionIt,
                                 const FieldVector<Scalar,dim>& xi) const
    {
        FieldVector<RT,m> values(0);
        values[0] = 1.013e5 + (depthBOR_ - x[2]) * 1037.24 * 9.81; // brine density for salinity=0.1
        values[1] = 0.0;
    ...
    virtual FieldVector<RT,m> neumann(const FieldVector<Scalar,dim>& x, const Entity& e,
    ...
    ProblemBenchmark3_2pni(Fluid& liq1, Fluid& liq2, Matrix2p<Grid, RT>& soil,
                           TwoPhaseRelations<Grid, RT>& law = *(new TwoPhaseRelations<Grid, RT>))
        : TwoPhaseHeatProblem<Grid,RT>(liq1, liq2, soil, law)
    {
        depthBOR_ = 3238.20;
    }
