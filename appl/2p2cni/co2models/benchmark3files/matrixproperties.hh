// $Id: matrixproperties.hh 1327 2009-03-02 14:26:47Z lauser $
...
public:
    ...
    const virtual FieldMatrix<DT,n,n> &K (const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi) const
    {
...
/*!
 * \brief Soil Class for reading soil properties from external file
 *
 * Can be used if RandomPermeability class is not needed.
 * To be able to read soil parameters that are dependent on the globalIdx
 * K, porosity, heatCap and heatCond are called with an additional
 * local index argument
 *
 */

template<class G, class RT>
class HeterogeneousSoil2: public Matrix2p<G,RT>
{
public:
    typedef typename G::Traits::template Codim<0>::Entity Entity;
    typedef typename G::ctype DT;
    ...
    const virtual FieldMatrix<DT,n,n> &K (const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi, const int idx) const
    {
        return K_;
    }

    virtual double porosity(const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi, const int idx) const
    {
        return 0.3;
    }
    ...
    virtual double Sr_n(const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi, const double T = 283.15) const
    {
        ...
        /* ATTENTION: define heat capacity per cubic meter! Be sure, that it corresponds to porosity!
     * Best thing will be to define heatCap = (specific heatCapacity of material) * density * porosity*/
    virtual double heatCap(const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi, const int idx) const
    {
        return 790 /* spec. heat cap. of granite */
        * 2700 /* density of granite */
    ...
    virtual double heatCond(const FieldVector<DT,n>& x, const Entity& e, const FieldVector<DT,n>& xi, const double sat, const int idx) const
    {
        static const double lWater = 0.6;
        static const double lGranite = 2.8;
        double poro = porosity(x, e, xi, idx);
        double lsat = pow(lGranite, (1-poro)) * pow(lWater, poro);
        double ldry = pow(lGranite, (1-poro));
        return ldry + sqrt(sat) * (ldry - lsat);
    }
    ...
    HeterogeneousSoil2(const G& g,const bool read, const char* name)
    :Matrix2p<G,RT>()
    {
        K_ = 0;
        for(int i = 0; i < n; i++)
