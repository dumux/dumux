...
public:
    ...
    void computeFlux(SolutionVector &flux, int faceId) const
    {
        ...
        Tensor K         = this->problem_.K(this->curElement_(), i);
        const Tensor &Kj = this->problem_.K(this->curElement_(), j);
        harmonicMeanK_(K, Kj);
    ...
    void addVtkFields(MultiWriter &writer, const SpatialFunction &globalSol)
    {
        ...
        ScalarField *porosity    = writer.template createField<Scalar, 1>(numVertices);
        ScalarField *permeability = writer.template createField<Scalar, 1>(numVertices);
        ...
        for (; elementIt != endit; ++elementIt)
        {
            ...
            for (int i = 0; i < numLocalVerts; ++i)
            {
                ...
                (*porosity)[globalIdx] = this->problem_.porosity(this->curElement_(), i);
                (*permeability)[globalIdx] = this->problem_.K(this->curElement_(), i)[0][0];
            };
        ...
        writer.addVertexData(porosity, "porosity");
        writer.addVertexData(permeability, "K in x direction");
