...
#ifndef DUMUX_MY_2P2C_LOCAL_RESIDUAL_HH
#define DUMUX_MY_2P2C_LOCAL_RESIDUAL_HH
...
// implement new TypeTag MyLocalResidualParams
namespace Properties
{
NEW_TYPE_TAG(MyLocalResidualParams);
NEW_PROP_TAG(ProblemEnableDiffusion);
SET_BOOL_PROP(MyLocalResidualParams, ProblemEnableDiffusion, true);
...
class MyTwoPTwoCLocalResidual: public GET_PROP_TYPE(TypeTag, BaseLocalResidual)
{
 protected:
    ...
    static constexpr unsigned int replaceCompEqIdx = GET_PROP_VALUE(TypeTag, ReplaceCompEqIdx);
 ...
 public:
    ...
    MyTwoPTwoCLocalResidual()
    {
        ...
        enableDiffusion_ = GET_PARAM_FROM_GROUP(TypeTag, bool, Problem, EnableDiffusion);
    }
    ...
    void computeFlux(PrimaryVariables &flux, const int fIdx, bool onBoundary=false) const
    {
        ...
        // TODO: dumux-course-task
        // only add diffusive fluxes if diffusion is enabled
        if (enableDiffusion_)
            computeDiffusiveFlux(flux, fluxVars);
    }
 ...
 private:
    ...
    // TODO: dumux-course-task
    // add private member enableDiffusion_
