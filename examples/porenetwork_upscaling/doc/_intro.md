...
Darcy (interinsic) single-phase permeability $`\mathbf{K}`$ [m$`^2`$] using creeping flow simulation, Forchheimer permeability $`\mathbf{K}`$ [m$`^2`$] and Forchheimer coefficient $`\mathbf{\beta}`$ [m$`^{-1}`$] using non-creeping flow simulation.
...
Non-creeping flow simulation additionally gives you Forchheimer permeability and coefficient for each spatial direction as in the example below for the x-direction:
...
Furthermore, the ratio of apparent permeability to Darcy permeability is plotted versus Forchheimer number for three spatial dimension as figure 2 shows.
...
In this example we are using the single-phase pore-network model of DuMu<sup>x</sup>. In pore network model We require mass conservation at each pore body $`i`$:
...
In simulation of non-creeping flow, to capture inertial effects in fluid flow, an extension of Hagen-Poiseuille-type law which include expansion and contraction of flow moving from a throat to a pore and vice versa is used. 
