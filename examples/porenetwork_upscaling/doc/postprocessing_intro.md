...
![Pipeline Browser](img/doc_pnm_visualization_pipelineBrowser.png)
1. Choose the _Glyph_ filter either by clicking on the respective icon (the name will appear if the cursor is moving over it) ![icon](img/doc_pnm_visualization_glyphSymbol.png) 
...
2. In the _Scale_ section, you can modify how large the spheres should be displayed. 
   - For showing all spheres with the same size, choose for the _Scale Array_ -> _No scale array_, then adapt the _Scale factor_ by clicking on the icon ![reset](img/doc_pnm_visualization_resetValue.png) for resetting the value using current data values. You can also give a concrete value, but make sure it is not too small and not too large. Otherwise you won't be able to see something.
   ![scale](img/doc_pnm_visualization_scale.png)
...
1. Select your input file (.pvd or .vtp) in the Pipeline Browser 
![Pipeline Browser](img/doc_pnm_visualization_pipelineBrowser.png)
2. Choose the _Tube_ filter by going to the _Filters_ -> _Alphabetical_ -> _Tube_
3. In the _Properties_ panel: increase the _Number of Sides_ to e.g. 10 for a smoother reprentation of the tubes. 
![sides](img/doc_pnm_visualization_tubeSides.png)
4. Leave everything else and click _Apply_.
...
![pipeline](img/doc_pnm_visualization_pipelineBrowser2.png)
