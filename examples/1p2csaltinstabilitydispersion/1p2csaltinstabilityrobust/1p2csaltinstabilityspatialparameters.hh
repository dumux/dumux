...
public:
    ...
    : ParentType(fvGridGeometry)
    {
        ...
        perm_ = 1.019368e-8;
        porosity_ = 0.35;
        dispTensor_.resize(fvGridGeometry->gridView().size(0),0.0);
    }
    ...
    void setDispTensors (const std::vector<Dune::FieldMatrix<Scalar,dimWorld,dimWorld>>& dispTensor)
    {
      dispTensor_ = dispTensor;
    }

    const Dune::FieldMatrix<Scalar,dimWorld,dimWorld>& dispTensor (const Element& element) const
    {
      return dispTensor_[this->fvGridGeometry().elementMapper().index(element)];
    }
...
private:
    Scalar perm_;
    Scalar porosity_;
    std::vector<Dune::FieldMatrix<Scalar,dimWorld,dimWorld>> dispTensor_;
};
